#!/usr/bin/env bash
# FR3 phase-1 excluded noncampaign deployment smoke on Rorqual, v2.
#
# This runner:
...
#
# v2 repairs the portable-token checksum and diagnostic-retrieval defects.
set -Eeuo pipefail
...
echo "================================================================="
echo "FR3 PHASE-1 RORQUAL NONCAMPAIGN DEPLOYMENT SMOKE V2"
echo "================================================================="
...
echo "Portable-token/retrieval recovery: ENABLED"
...
if ! git diff --cached --quiet; then
  ...
  git commit -m "Repair and rerun excluded Rorqual phase1 deployment smoke"
  git push
fi
...
PY
TOKEN_DIGEST="$(sha256sum "$TOKEN" | awk '{print $1}')"
...
# Prove locally that the checksum sidecar is portable and basename-only.
(
  cd "$(dirname "$TOKEN")"
  sha256sum -c "$(basename "$TOKEN_SHA")"
)
TOKEN_LISTED_NAME="$(awk '{print $2}' "$TOKEN_SHA")"
[[ "$TOKEN_LISTED_NAME" == "$TOKEN_BASENAME" ]] || {
  echo "ERROR: token checksum sidecar is not basename-only"
  echo "Listed name: $TOKEN_LISTED_NAME"
  exit 70
}
...
echo "Live token SHA-256: $TOKEN_DIGEST"
echo "Portable token checksum sidecar: PASS"
...
# Capture remote failure in an if-condition so the global ERR trap cannot skip
# compact diagnostic retrieval.
...
TOKEN_LISTED_NAME="$(awk '{print $2}' "$TOKEN_SHA_NAME")"
[[ "$TOKEN_LISTED_NAME" == "$TOKEN_NAME" ]] || {
  echo "ERROR: remote token checksum sidecar is not portable"
  echo "Expected name: $TOKEN_NAME"
  echo "Listed name:   $TOKEN_LISTED_NAME"
  exit 22
}
sha256sum -c "$TOKEN_SHA_NAME"
echo "REMOTE PORTABLE TOKEN CHECKSUM: PASS"
...
echo "Remote orchestration exit captured: $REMOTE_EXIT"
echo "Diagnostic retrieval will run regardless of remote success/failure."
