# FR3 phase-1 Rorqual noncampaign deployment smoke v2
...
Version 1 reached Rorqual but stopped before `sbatch` because its token
checksum sidecar embedded an absolute local WSL path. Version 2 writes a
basename-only checksum, validates it locally and remotely, and always retrieves
the compact diagnostic after a remote failure. The immutable scientific job
package is unchanged, and the failed v1 attempt submitted no Rorqual job.
