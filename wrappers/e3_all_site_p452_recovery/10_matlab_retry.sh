...
[[ -f "$MATLAB_EXE" ]] || { echo "MATLAB missing: $MATLAB_EXE"; exit 2; }
[[ -d "$P452_WINDOWS_ROOT/matlab" ]] || {
  echo "Validated P.452 folder missing: $P452_WINDOWS_ROOT/matlab"
  exit 3
}

echo "=== MATLAB table-construction syntax probe ==="
"$MATLAB_EXE" -wait -batch \
  "a=(1:3)'; b=string(['x';'y';'z']); t=table(a,b,'VariableNames',{'a','b'}); assert(height(t)==3 && width(t)==2); fprintf('MATLAB TABLE NAME-VALUE PROBE: PASS\n');"
...
echo "WRAPPER 10 MATLAB RETRY: PASS"
