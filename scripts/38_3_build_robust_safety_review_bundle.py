#!/usr/bin/env python3
"""Build the compact robust delayed-safety milestone review bundle."""
...
def main() -> int:
    ...
    files = [
        ...
        if path.is_file()
        and path != output
        and path.name != "ROBUST_PRIMARY_ACTIONS.npz"
    ]
    for path in [
        ROOT / "config/robust_safety_baselines_v1.json",
        ROOT / "docs/DELAYED_REACHABILITY_SAFETY_THEOREM.md",
        ROOT / "src/fr3_cbf/robust_delayed_safety.py",
        ROOT / "scripts/38_0_run_robust_safety_baselines.py",
        ROOT / "scripts/38_1_validate_robust_safety_baselines.py",
    ...
    print("ROBUST DELAYED SAFETY REVIEW BUNDLE: PASS")
