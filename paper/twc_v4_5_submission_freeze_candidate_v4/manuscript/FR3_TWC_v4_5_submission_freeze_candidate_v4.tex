...
FR3 studies emphasize the propagation, array, and coexistence questions that arise above conventional sub-7-GHz bands~\cite{kang2024upper,chamberlain2024sharing}. The cellular realization uses the pinned Sionna 2.0.1 UMa implementation, which Sionna documents as based on 3GPP TR~38.901~\cite{3gpp38901,hoydis2022sionna,sionna201docs}. We do not claim an independent clause-by-clause certification of that implementation to TR~38.901 V19.4.0. Incumbent coupling and engineering criteria use the explicitly frozen ITU-R roles in Section~\ref{sec:model}~\cite{iturp452,itursa1027,itursa509,iturf758}. Table~\ref{tab:positioning} summarizes the resulting position: the work is neither static exclusion nor centralized beam optimization, but a predictive bounded-local hierarchy with explicit failure semantics.

\input{tables/positioning.tex}

\section{System and Protection Model}
\label{sec:model}
\subsection{Network and practical array}
The evaluated platform has 19 sites, three sectors per site, 57 sectors, and 228 users. Each sector uses an $8\times8$ panel of dual-polarized element locations: 64 locations, 128 Sionna antenna ports, half-wavelength spacing at 8.15~GHz, and a generic practical mapping with 64 RF chains, 32 disjoint dual-polarized subarrays, and 6-bit analog phase control. Local digital RZF is computed after the analog network. The observed minimum remaining digital dimension in the frozen architecture is 58, and no local-rank failure occurs in the evaluated topology.
...
\input{tables/system_parameters_expanded.tex}
...
The protected service is an EESS space-to-Earth receiving scenario at 8.15~GHz, inside the 8~025--8~400~MHz band addressed by Rec. ITU-R SA.1027-6. Table~1 of that Recommendation gives the terrestrial-path single-entry criteria used here: $-150$~dBW/10~MHz for $p=20\%$ and $-133$~dBW/10~MHz for $p=0.005\%$, with both criteria required and a minimum satellite elevation of $5^\circ$~\cite{itursa1027}. The evaluated window begins at $5.012^\circ$. We test the aggregate 57-sector network as one engineered interference entry; this corresponds algebraically to $n=1$ in the SA.1023 apportionment equations, but is a conservative study convention rather than a normative assertion that the network is physically one transmitter~\cite{itursa1023}.

The public Gatineau reference geometry and NRCan MRDEM terrain are used without asserting that the modeled antenna parameters describe a currently operational receiver. The P.452-18 implementation passed all 17 official SG3 validation examples with deviation below $10^{-6}$~dB and an all-site 19-link/798-row reconstruction audit~\cite{iturp452,iturp452validation}. Terrain heights are used as both terrain and representative clutter heights ($g=h$); no separate P.2108 loss is added, avoiding clutter double counting. For the predominantly open/rural profiles, this choice tends to omit rather than add clutter shielding and is therefore protective with respect to interference. Because SA.1027 states unqualified percentage-of-time criteria, the $p$ values are paired directly with P.452 annual-average-year outputs; no worst-month conversion is applied.

The primary receive pattern is the SA.509-3 multiple-entry pattern because the interference is aggregated over 57 sectors. SA.509 is titled for space-research earth stations and radio astronomy, not specifically EESS, and no measured station pattern is available; it is therefore an explicit engineering proxy rather than a prescribed EESS antenna~\cite{itursa509}. The assumed $13.00$-m dish has $G_0=59.04$~dBi and $D/\lambda=353.41$, satisfying SA.509's $D/\lambda\ge100$ applicability condition. Over the protected window all off-axis angles exceed $8.03^\circ$, where the single-entry curve is exactly 3~dB above the multiple-entry curve.

Table~\ref{tab:sa509-sensitivity} reports a conservative peak-envelope stress by applying the 3-dB pattern offset, in addition to the already-applied coupling uplift $\chi$, to the \emph{frozen} holdout actions. The long criterion then fails for 83,859 of 88,140 evaluated seconds (95.1\%) across all 150 cells, while the short criterion remains safe; up to 3.01~dB additional attenuation would be needed for the same actions. This sensitivity is not a re-optimized single-entry controller result and is not called a certified bound for the actual station. The zero-violation headline is consequently conditional on the declared aggregate multiple-entry proxy. These are deterministic engineering tests, not calibration or regulatory-compliance evidence.

SRSP-307.7 documents co-band Canadian fixed-service systems in 7~725--8~275~MHz, but fixed-service receivers are outside the protected-service set of this study; the only protected receiver modeled and evaluated is the EESS earth station~\cite{ised3077}.

\input{tables/sa509_pattern_sensitivity.tex}
...
The cellular topology is generated once per seed with the pinned Sionna 2.0.1 UMa implementation and reused across all five fixed pass blocks and all methods. This is a version-bound TR~38.901-based engineering realization, not an independent V19.4.0 certification or a 3GPP calibration campaign. The pass blocks are deterministic geometry/load blocks rather than independent draws from an orbital-pass population. The control update is 5 s, the command delay is one update, and attenuation changes by at most 3 dB per update. The development campaign uses seeds 44000--44029. Candidate v4.5 is frozen after that development and then evaluated, without adaptation, on fresh seeds 44030--44059.
...
\input{tables/holdout_key_results_v2.tex}
\input{tables/development_holdout.tex}

The final moving-PF advantage over safe static is $0.2505$ and its 95\% seed-cluster interval is $[0.2013,0.3102]$. Dividing the sum-log effect over 228 users gives an approximate $0.110\%$ increase in the $\epsilon$-shifted geometric mean of final moving rates; this is not an exact raw-rate geometric-mean claim. The magnitude is modest in aggregate-rate terms, so the headline is the joint utility, safety, and floor-burden result rather than a large capacity gain. The corresponding duration-weighted advantage is $0.2088$ with interval $[0.1767,0.2415]$. Relative to the reviewed predictive controller, the final and duration-weighted effects are $0.0563$ and $0.0360$, with both 95\% intervals above zero. The development-to-holdout reduction in effect size is moderate and expected after freezing; the independent holdout remains statistically decisive.
...
\input{tables/method_comparison_full.tex}
...
\input{tables/development_ablation.tex}
...
\input{tables/runtime_distribution.tex}
...
\item P.452/SA.1027/SA.509 are used in a documented engineering scenario. SA.509 is an explicitly scoped proxy, and the zero-violation result is conditional on its multiple-entry curve; the single-entry frozen-action stress is not safe. The results are not a licensing or regulatory-compliance certificate.
