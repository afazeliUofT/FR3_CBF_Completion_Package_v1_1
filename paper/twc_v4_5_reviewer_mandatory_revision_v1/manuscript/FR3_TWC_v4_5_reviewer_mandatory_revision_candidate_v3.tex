...
The public Gatineau reference geometry and NRCan MRDEM terrain are used without asserting that the modeled antenna parameters describe a currently operational receiver. The P.452-18 implementation passed all 17 official SG3 validation examples with deviation below $10^{-6}$~dB and an all-site 19-link/798-row reconstruction audit~\cite{iturp452,iturp452validation}. Terrain heights are used as both terrain and representative clutter heights ($g=h$); no separate P.2108 loss is added, avoiding clutter double counting. The $p$ values are interpreted directly as annual percentages for an average year; no worst-month conversion is applied.
...
Table~\ref{tab:sa509-sensitivity} reports a conservative peak-envelope stress by applying this 3-dB uplift to the \emph{frozen} holdout actions. The long criterion then fails in all 150 cells, while the short criterion remains safe; up to 3.01~dB additional attenuation would be needed for the same actions. This sensitivity is not a re-optimized single-entry controller result and is not called a certified bound for the actual station. The zero-violation headline is consequently conditional on the declared aggregate multiple-entry proxy. These are deterministic engineering tests, not calibration or regulatory-compliance evidence.
