#!/usr/bin/env python3
"""Issue a short-lived original-package token used only by the seed-44052 job."""
...
def main() -> int:
    ...
    if completion["existing_candidate_source_manifest_sha256"] != job["candidate_v4_5"]["source_manifest_sha256"]:
        raise RuntimeError("completion/candidate source manifest mismatch")
    if completion["scope"]["rerun_seed_list"] != [44052]:
        raise RuntimeError("completion rerun scope is not exactly seed 44052")
    expiry = datetime.now(timezone.utc) + timedelta(hours=12)
    token = {
        "schema_version": 1,
        "authorization": "EXECUTE_FRESH_V4_5_HOLDOUT_RORQUAL_V1",
        "authorization_package_sha256": "HOLDOUT_COMPLETION_REPAIR_R1",
        ...
        "execution_wrapper_restriction": "RUN_ONLY_SEED_44052_REUSE_EXISTING_CHANNEL",
    ...
    print("SEED44052_COMPLETION_AUTHORIZATION_ISSUED=PASS")
    print("NATIVE_AUTHORIZATION_GUARD=PASS")
    print("AUTHORIZED_EXECUTION_WRAPPER_SCOPE=SEED_44052_ONLY")
