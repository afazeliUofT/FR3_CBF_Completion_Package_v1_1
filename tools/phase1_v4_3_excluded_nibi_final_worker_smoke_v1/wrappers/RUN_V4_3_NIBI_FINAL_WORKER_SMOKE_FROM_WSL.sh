...
REQUIRED_ANCESTOR="cc1c911ef5592a2c22834d7fb3e6550bbd4ab2ab"
TOOL_DIR="tools/phase1_v4_3_excluded_nibi_final_worker_smoke_v1"
STAMP="$(date -u +%Y%m%d_%H%M%S)"
LOCAL_RETURN="$DOWNLOADS/FR3_V4_3_NIBI_FINAL_WORKER_SMOKE_RETURN_$STAMP"
mkdir -p "$LOCAL_RETURN"
LOCAL_LOG="$LOCAL_RETURN/LOCAL_WSL_ORCHESTRATOR.log"
exec > >(tee "$LOCAL_LOG") 2>&1

CONTROL_PATH="/tmp/fr3-v43-nibi-final-$$-%C"
SSH_OPTS=(
...
JOB_ID="UNKNOWN"
RETURN_ZIP=""
NEXT_GATE="DIAGNOSE_FINAL_CAMPAIGN_WORKER_OR_NIBI_CHANNEL_REPRODUCTION_BEFORE_ANY_CAMPAIGN_AUTHORIZATION"
...
printf '%s\n' \
  "=================================================================" \
  "FR3 V4.3 — EXCLUDED NIBI FINAL CAMPAIGN-WORKER SMOKE" \
  "=================================================================" \
...
EXISTING_SMOKE_EVIDENCE="$(
  find "$SOURCE_CLONE/evidence" -mindepth 1 -maxdepth 1 -type d \
    -name 'phase1_v4_3_nibi_final_worker_smoke_v1_job_*' -print 2>/dev/null \
    | sort | head -n1
)"
...
if git -C "$SOURCE_CLONE" diff --cached --quiet; then
  ...
  git -C "$SOURCE_CLONE" commit -m "Add excluded Nibi final campaign-worker smoke for candidate-v4.3"
...
PREFLIGHT="$(
  ssh "${SSH_OPTS[@]}" "$NIBI_HOST" '
    ...
    [[ "$host" == *nibi* ]]
    [[ -n "${SCRATCH:-}" ]]
    echo "NIBI_CONNECTION=PASS"
    echo "NIBI_REMOTE_USER=$USER"
    echo "NIBI_REMOTE_HOST=$host"
    echo "NIBI_REMOTE_SCRATCH=$SCRATCH"
...
REMOTE_ENV="$REMOTE_SCRATCH/FR3_PHASE1_NIBI_ENV_77bec1efa2ea"
REMOTE_SCRIPT="$PACKAGE_ROOT/wrappers/REMOTE_ORCHESTRATE_V4_3_NIBI_FINAL_WORKER_SMOKE.sh"
...
scp "${SCP_OPTS[@]}" -p "$ARCHIVE_PATH" "$REMOTE_SCRIPT" "$NIBI_HOST:$REMOTE_TRANSFER/"
ssh "${SSH_OPTS[@]}" "$NIBI_HOST" \
...
REMOTE_COMMAND="$(
  python - \
    ...
    "$REMOTE_TRANSFER" "$REMOTE_RETURN" "$REMOTE_RUN" "$REMOTE_ENV" \
    "$SOURCE_COMMIT" "168" "$REMOTE_TRANSFER/$(basename "$REMOTE_SCRIPT")" <<'PY'
import shlex,sys
(zip_name,zip_sha,transfer,returned,run,env,commit,ttl,script)=sys.argv[1:]
values={
...
if ssh "${SSH_OPTS[@]}" "$NIBI_HOST" "$REMOTE_COMMAND" \
...
if [[ -n "$REMOTE_FILES" ]]; then
  while IFS= read -r name; do
    [[ -n "$name" ]] || continue
    if scp "${SCP_OPTS[@]}" -p "$NIBI_HOST:$REMOTE_RETURN/$name" "$LOCAL_RETURN/"; then
      :
    else
      RETRIEVAL_RC=$?
    fi
...
RETURN_ZIP="$(
  find "$LOCAL_RETURN" -maxdepth 1 -type f \
    -name 'FR3_NIBI_V4_3_FINAL_WORKER_SMOKE_43999_*.zip' \
...
if [[ -n "$RETURN_ZIP" && -f "$RETURN_ZIP" ]]; then
  JOB_ID="$(basename "$RETURN_ZIP" | sed -n 's/^FR3_NIBI_V4_3_FINAL_WORKER_SMOKE_43999_\([0-9][0-9]*\)\.zip$/\1/p')"
...
environment_path=root/'provenance/NIBI_H100_ENVIRONMENT.json'
...
lines=[
    ...
    emit('NIBI_H100_FINAL_WORKER_ENVIRONMENT_GATE','PASS' if environment.get('status')=='PASS_NIBI_H100_FINAL_WORKER_ENVIRONMENT' else 'FAIL_OR_MISSING'),
    emit('NIBI_EXACT_REFERENCE_ENVIRONMENT_GATE',environment.get('exact_reference_environment_gate','NOT_AVAILABLE')),
...
PY
  ...
  EVIDENCE_DIR="$EVIDENCE_CLONE/evidence/phase1_v4_3_nibi_final_worker_smoke_v1_job_${JOB_ID}"
  [[ ! -e "$EVIDENCE_DIR" ]] || {
  ...
  for relative in \
    ...
    provenance/NIBI_H100_ENVIRONMENT.json \
...
PY
  git -C "$EVIDENCE_CLONE" add "evidence/phase1_v4_3_nibi_final_worker_smoke_v1_job_${JOB_ID}"
  git -C "$EVIDENCE_CLONE" diff --cached --check
  git -C "$EVIDENCE_CLONE" commit -m "Collect candidate-v4.3 Nibi final-worker smoke evidence"
...
printf '%s\n' \
  "=================================================================" \
  "FR3 V4.3 — NIBI FINAL WORKER SMOKE FINAL REPORT" \
  "=================================================================" \
