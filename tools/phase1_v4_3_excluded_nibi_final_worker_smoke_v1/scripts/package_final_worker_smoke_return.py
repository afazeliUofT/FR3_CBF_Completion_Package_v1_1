...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="fr3-final-worker-return-") as temp_name:
        temp = Path(temp_name)
        payload = temp / f"FR3_NIBI_V4_3_FINAL_WORKER_SMOKE_43999_{args.job_id}"
        payload.mkdir()
        ...
        status = (
            "PASS_EXCLUDED_NIBI_FINAL_WORKER_SMOKE_RETURN_READY"
            if state_completed and all_exit_pass and scientific_pass
            else "DIAGNOSTIC_EXCLUDED_NIBI_FINAL_WORKER_SMOKE_RETURN_READY"
        )
        metadata = {
            "schema_version": 1,
            ...
            "next_gate": (
                "INDEPENDENTLY_REVIEW_NIBI_FINAL_WORKER_SMOKE_THEN_ISSUE_SEPARATE_FULL_CAMPAIGN_AUTHORIZATION"
                if status == "PASS_EXCLUDED_NIBI_FINAL_WORKER_SMOKE_RETURN_READY"
                else "DIAGNOSE_FINAL_CAMPAIGN_WORKER_OR_NIBI_CHANNEL_REPRODUCTION_BEFORE_ANY_CAMPAIGN_AUTHORIZATION"
            ),
        }
