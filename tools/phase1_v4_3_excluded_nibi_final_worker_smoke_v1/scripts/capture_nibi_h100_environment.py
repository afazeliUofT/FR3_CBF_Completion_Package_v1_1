#!/usr/bin/env python3
"""Capture and validate the final-worker Nibi H100 runtime environment."""
...
def main() -> int:
    ...
    if mismatches:
        raise RuntimeError(f"Nibi environment version mismatch: {mismatches}")
    if platform.python_version() != "3.12.4":
        raise RuntimeError(
            "Nibi Python mismatch: "
            f"{platform.python_version()} != 3.12.4"
        )
    if str(torch.version.cuda) != "12.6":
        raise RuntimeError(
            f"Nibi CUDA build mismatch: {torch.version.cuda} != 12.6"
        )

    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is not visible in the Nibi H100 smoke allocation")
    ...
    print("NIBI_H100_FINAL_WORKER_ENVIRONMENT_GATE=PASS")
    print("NIBI_EXACT_REFERENCE_ENVIRONMENT_GATE=PASS")
    print(f"NIBI_GPU_NAME={name}")
    print(f"NIBI_GPU_MEMORY_GIB={memory / 1024**3:.6f}")
    return 0
