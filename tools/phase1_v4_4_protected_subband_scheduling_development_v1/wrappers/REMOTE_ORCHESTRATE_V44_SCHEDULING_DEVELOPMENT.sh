...
RUN_ROOT="$SCRATCH_ROOT/FR3_V44_PROTECTED_SUBBAND_SCHEDULING_DEVELOPMENT_${RUN_TAG}"
...
printf '%s\n' \
  ...
  "EXECUTION_STAGE=V4_4_PROTECTED_SUBBAND_SCHEDULING_DEVELOPMENT" \
...
if [[ -f "$STATE_FILE" ]]; then
  ...
  active="$(squeue -u "$USER" -h -o '%i|%j|%T' | grep -E '\|(fr3-v44-sched|fr3-v44-sched-merge)\|' || true)"
  if [[ -n "$active" ]]; then
...
record={
 ...
 'status':'PASS' if int(sys.argv[3])==0 else 'SCIENTIFIC_OR_INFRASTRUCTURE_REVIEW_REQUIRED',
}
...
#!/usr/bin/env bash
#SBATCH --job-name=fr3-v44-sched-merge
...
EOF_STATE
  printf '%s\n' \
    "V44_SCHEDULING_DEVELOPMENT_SUBMISSION=PASS" \
    "ARRAY_JOB_ID=$ARRAY_JOB_ID" \
    "MERGE_JOB_ID=$MERGE_JOB_ID" \
    "WORKER_ARRAY=0-10%4"
fi
...
name=f"FR3_RORQUAL_V4_4_PROTECTED_SUBBAND_SCHEDULING_DEVELOPMENT_{array_job_id}_EMERGENCY"
