...
This module is a new candidate-v4.3 diagnostic/repair layer.  It does not modify
...
# Solver-side certification controls. These tighten the optimization model;
# they do not relax any scientific floor/EESS comparison tolerance.
SOLVER_CONSTRAINT_RESERVE = 1e-8
SOLVER_PRIMAL_FEASIBILITY_TOLERANCE = 1e-9
SOLVER_DUAL_FEASIBILITY_TOLERANCE = 1e-9
SOLVER_NORMALIZED_VERIFICATION_TOLERANCE = 1e-10
...
class RepairChoice:
    ...
    strict_post_mode_sparse_stream: SolverOutcome
    q0_nominal_envelope_sparse_stream: SolverOutcome
...
def stream_power_envelopes(
    matrices: object,
    q_db: np.ndarray,
) -> tuple[np.ndarray, np.ndarray]:
    """Return actual post-mode and nominal ``q=0`` stream powers.

    The reviewed mode decomposition partitions the protected precoder into
    mutually orthogonal perpendicular and two polarization components.  A
    nonnegative attenuation command therefore cannot increase conducted power.
    The deployable v4.3 repair uses post-mode powers on both sides of its
    strict sector-power constraint. The original ``q=0`` powers are returned
    only for a broader nondeployable diagnostic oracle.
    """
    q = np.asarray(q_db, dtype=np.float64)
    if not np.all(np.isfinite(q)) or np.any(q < -1e-12):
        raise ValueError("mode attenuation q_db must be finite and nonnegative")
    actual_precoder = np.asarray(mode_adjusted_precoder(matrices, q))
    nominal_precoder = np.asarray(
        mode_adjusted_precoder(matrices, np.zeros_like(q, dtype=np.float64))
    )
    actual = np.sum(np.abs(actual_precoder) ** 2, axis=1).astype(np.float64)
    nominal = np.sum(np.abs(nominal_precoder) ** 2, axis=1).astype(np.float64)
    if actual.shape != nominal.shape or actual.ndim != 2:
        raise ValueError("invalid stream-power envelope shape")
    if not np.all(np.isfinite(actual)) or not np.all(np.isfinite(nominal)):
        raise ValueError("stream-power envelope contains a non-finite value")
    if np.any(actual < -1e-14) or np.any(nominal < -1e-14):
        raise ValueError("stream-power envelope contains a negative value")
    tolerance = 1e-11 * np.maximum(1.0, nominal)
    if np.any(actual > nominal + tolerance):
        worst = float(np.max(actual - nominal))
        raise RuntimeError(
            "post-mode stream power exceeds the q=0 nominal envelope; "
            f"the reviewed orthogonal mode-decomposition contract failed "
            f"(maximum excess {worst:.6e})"
        )
    return actual, nominal


def stream_leakage_and_power_envelopes(
    matrices: object,
    q_db: np.ndarray,
    steering_pol1: np.ndarray,
    steering_pol2: np.ndarray,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Return EESS leakage, actual power, and the nominal ``q=0`` envelope.

    All returned arrays have shape ``[sector,stream]``.  Leakage and actual
    power are evaluated after the frozen mode command.  The third array is the
    immutable full-mode physical sector-power reference used only on the
    right-hand side of candidate redistribution constraints.
    """
    ...
    leakage = (np.abs(c1) ** 2 + np.abs(c2) ** 2).astype(np.float64)
    actual_power, nominal_q0_power = stream_power_envelopes(matrices, q_db)
    return leakage, actual_power, nominal_q0_power
...
def stream_leakage_and_power(
    ...
    """Backward-compatible pair of post-mode leakage and actual power."""
    leakage, actual_power, _nominal_q0_power = (
        stream_leakage_and_power_envelopes(
            matrices, q_db, steering_pol1, steering_pol2
        )
    )
    return leakage, actual_power
...
def _row_normalized_system(
    system: LinearSystem,
    *,
    constraint_reserve: float = 0.0,
) -> LinearSystem:
    """Return a dimensionless row-scaled system with optional tightening.

    Each nontrivial inequality is divided by ``max(||a_i||_inf, |b_i|)``.
    A positive reserve is then subtracted from the normalized right-hand side.
    This creates a solver-side interior certificate without changing the
    exact nonlinear floor/EESS tolerances used by :func:`exact_audit`.
    """
    reserve = float(constraint_reserve)
    if not math.isfinite(reserve) or reserve < 0.0:
        raise ValueError("constraint_reserve must be finite and nonnegative")
    a = np.asarray(system.a_ub, dtype=np.float64)
    b = np.asarray(system.b_ub, dtype=np.float64)
    if not len(b):
        return LinearSystem(a.copy(), b.copy(), system.labels)
    row_a = np.max(np.abs(a), axis=1)
    scale = np.maximum(np.maximum(row_a, np.abs(b)), np.finfo(np.float64).tiny)
    normalized_a = a / scale[:, None]
    normalized_b = b / scale
    active = row_a > 0.0
    normalized_b = normalized_b.copy()
    normalized_b[active] -= reserve
    return LinearSystem(normalized_a, normalized_b, system.labels)
...
def solve_continuous_l1(
    ...
    ``constraint_reserve`` is a dimensionless tightening applied after row
    normalization. A positive value is used only for deployable witness
    recovery. It is never used to claim infeasibility of the untightened
    scientific action class.
    """
    matrix = np.asarray(mapping, dtype=np.float64)
    fixed = np.asarray(offset, dtype=np.float64)
    ...
    original_system = mapped_system(stream_system, matrix, fixed)
    if extra_system is not None:
        ...
        original_system = combine_systems(original_system, extra_system)

    # Exact no-op shortcut. A reserve is unnecessary when the reviewed action
    # already passes the unchanged exact hard gates.
    baseline_excess = _maximum_excess(original_system, baseline)
    ...
    normalized_original = _row_normalized_system(original_system)
    solver_system = _row_normalized_system(
        original_system, constraint_reserve=reserve
    )

    # Variables are [decision, absolute-deviation].
    n = variable_count
    c = np.concatenate([np.zeros(n), weight])
    hard = csr_matrix(solver_system.a_ub)
    ...
    b_ub = np.concatenate([solver_system.b_ub, b_dev])
    ...
    if not result.success:
        if reserve > 0.0 and result.status == 2:
            status = "NO_CERTIFIED_WITNESS_AT_FIXED_RESERVE"
            message = (
                f"tightened normalized LP infeasible at reserve={reserve:.3e}; "
                "this does not prove infeasibility of the untightened class; "
    ...
    raw_excess = _maximum_excess(original_system, decision)
    normalized_excess = _maximum_excess(normalized_original, decision)
    if reserve > 0.0:
        certified = normalized_excess <= -0.5 * reserve
        status = (
            "OPTIMAL_CONTINUOUS_L1_CERTIFIED_RESERVE"
            if certified
            else "NUMERICAL_CERTIFICATE_MARGIN_FAIL"
        )
    else:
        certified = (
            normalized_excess <= SOLVER_NORMALIZED_VERIFICATION_TOLERANCE
        )
        status = (
            "OPTIMAL_CONTINUOUS_L1"
            if certified
            else "NUMERICAL_VERIFICATION_FAIL"
        )
    message = (
        f"{result.message}; normalized_original_max_excess="
        f"{normalized_excess:.17g}; constraint_reserve={reserve:.17g}"
    )
    return SolverOutcome(
        status=status,
        feasible=bool(certified),
        optimal=bool(certified),
        decision=decision,
        stream_scale=stream,
        objective=float(result.fun),
        message=message,
        maximum_constraint_excess=raw_excess,
...
def local_sector_power_budget_system(
    *,
    ...
    envelope_stream_power: np.ndarray,
    baseline_sector_scale: np.ndarray,
    constrained_sectors: Sequence[int],
    label_prefix: str = "q0_nominal_power_envelope",
) -> LinearSystem:
    """Return exact decision-space protected-power envelope rows.

    The left-hand side always uses *actual post-mode* conducted stream
    powers. The deployable v4.3 row also supplies post-mode powers as the
    right-hand-side envelope. Supplying original ``q=0`` nominal powers yields
    a broader diagnostic oracle that is explicitly nondeployable.
    """
    matrix = np.asarray(mapping, dtype=np.float64)
    fixed = np.asarray(offset, dtype=np.float64)
    actual = np.asarray(actual_stream_power, dtype=np.float64)
    envelope = np.asarray(envelope_stream_power, dtype=np.float64)
    baseline = np.asarray(baseline_sector_scale, dtype=np.float64)
    if (
        actual.ndim != 2
        or envelope.shape != actual.shape
        or baseline.shape != (actual.shape[0],)
    ):
        raise ValueError("invalid sector-power budget inputs")
    if matrix.shape[0] != actual.size or fixed.shape != (actual.size,):
        raise ValueError("mapping does not align with stream power")
    if (
        not np.all(np.isfinite(actual))
        or not np.all(np.isfinite(envelope))
        or np.any(actual < -1e-14)
        or np.any(envelope < -1e-14)
    ):
        raise ValueError("invalid actual/envelope stream power")
    rows: list[np.ndarray] = []
    rhs: list[float] = []
    labels: list[str] = []
    flat_actual = actual.reshape(-1)
    for sector in sorted(set(int(v) for v in constrained_sectors)):
        if sector < 0 or sector >= actual.shape[0]:
            raise ValueError("constrained sector out of range")
        selector = np.zeros_like(flat_actual)
        start = sector * actual.shape[1]
        stop = start + actual.shape[1]
        selector[start:stop] = flat_actual[start:stop]
        row = selector @ matrix
        bound = (
            baseline[sector] * float(np.sum(envelope[sector]))
            - float(selector @ fixed)
        )
        scale = max(
            float(np.max(np.abs(row))) if row.size else 0.0,
            abs(bound),
            1e-30,
        )
        rows.append(np.asarray(row, dtype=np.float64) / scale)
        rhs.append(float(bound) / scale)
        labels.append(f"{label_prefix}:sector={sector}")
    if not rows:
        return LinearSystem(
...
def repair_interval(
    *,
    ...
    leakage, stream_power, nominal_q0_stream_power = (
        stream_leakage_and_power_envelopes(
    ...
    strict_post_mode_sparse_outcome = SolverOutcome(
        status=(
        ...
        message=(
            ...
            if local_grid.feasible
            else "strict post-mode stream comparator not attempted"
        ),
        maximum_constraint_excess=local_grid.maximum_constraint_excess,
        changed_variable_count=local_grid.changed_variable_count,
    )
    q0_outcome = SolverOutcome(
        status=(
            "NOT_NEEDED_LOCAL_GRID_FEASIBLE"
            if local_grid.feasible
            else "NOT_ATTEMPTED"
    ...
    if not baseline_hard_pass and not local_grid.feasible:
        for count in neighbourhood_sizes:
            ...
            strict_post_mode_budget = local_sector_power_budget_system(
                mapping=mapping,
                offset=offset,
                actual_stream_power=stream_power,
                envelope_stream_power=stream_power,
                baseline_sector_scale=baseline_sector,
                constrained_sectors=critical,
                label_prefix="strict_post_mode_power_budget",
            )
            q0_envelope_budget = local_sector_power_budget_system(
                mapping=mapping,
                offset=offset,
                actual_stream_power=stream_power,
                envelope_stream_power=nominal_q0_stream_power,
                baseline_sector_scale=baseline_sector,
                constrained_sectors=critical,
                label_prefix="q0_nominal_power_envelope",
            )
            strict_post_mode_sparse_outcome = solve_continuous_l1(
                hard_stream,
                mapping,
                offset,
                base_decision,
                ...
                extra_system=strict_post_mode_budget,
                constraint_reserve=SOLVER_CONSTRAINT_RESERVE,
            )
            q0_outcome = solve_continuous_l1(
                hard_stream,
                mapping,
                offset,
                base_decision,
                ...
                upper,
                np.asarray(weights, dtype=np.float64),
                extra_system=q0_envelope_budget,
                constraint_reserve=SOLVER_CONSTRAINT_RESERVE,
            )
            selected_external = tuple(
                sorted(set(external) - set(critical))
            ...
            if strict_post_mode_sparse_outcome.feasible:
                break
    ...
    if frozen_grid.feasible and frozen_grid.decision is not None:
        ...
        strict_post_mode_sparse_outcome.feasible
        and strict_post_mode_sparse_outcome.stream_scale is not None
    ):
        global_stream = SolverOutcome(
            ...
            optimal=False,
            decision=None,
            stream_scale=np.asarray(strict_post_mode_sparse_outcome.stream_scale),
            objective=None,
            message="sparse local stream class is a subset of the global stream class",
            maximum_constraint_excess=(
                strict_post_mode_sparse_outcome.maximum_constraint_excess
            ),
            changed_variable_count=(
                strict_post_mode_sparse_outcome.changed_variable_count
            ),
        )
    ...
    if baseline_hard_pass:
        ...
        chosen_stream = np.asarray(strict_post_mode_sparse_outcome.stream_scale)
    else:
    ...
    return RepairChoice(
        ...
        strict_post_mode_sparse_stream=strict_post_mode_sparse_outcome,
        q0_nominal_envelope_sparse_stream=q0_outcome,
