"""Floor-first protected-subband scheduling for candidate v4.4.
...
def _sector_options(
    ...
    Singleton modes are generated only for users that actually violate the
    pre-repair floor.  A full-active mode protects nonviolating users in the
    same sector without exploding the mode count.
    """
    ...
    for local_stream, user in sorted(target_stream_to_user.items()):
...
def schedule_interval(
    *,
    ...
    """Try total guard caps 0, 2, 4, then an 8-sector diagnostic oracle."""
    ...
    for guard_limit, diagnostic in (
        (0, False),
        (2, False),
        (MAX_DEPLOYABLE_GUARD_SECTORS, False),
        (DIAGNOSTIC_GUARD_SECTORS, True),
    ):
        ...
        if outcome.feasible:
            return outcome
        if diagnostic and outcome.status == "DIAGNOSTIC_SCHEDULING_FEASIBLE":
            return outcome
