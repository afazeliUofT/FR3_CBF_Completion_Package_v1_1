...
methods, and then applies candidate-v4.1 only as a separately versioned fixed-q,
...
def load_worker(package_root: Path):
    ...
    return load_module(
        ...
        "phase1_seed_worker_candidate_v4_1_import",
    )
...
def classify_interval(choice: Any, optimistic: dict[str, Any], floor: float) -> str:
    ...
    if choice.sparse_stream.feasible:
        return "LOCAL_SECTOR_GRID_INFEASIBLE_POWER_PRESERVING_STREAM_REPAIR_FEASIBLE"
...
def main() -> int:
    ...
    try:
        ...
        repair = load_module(candidate_source, "fr3_floor_repair_candidate_v4_1")
        ...
        with tempfile.TemporaryDirectory(prefix="fr3_candidate_v4_1_passes_") as name:
            pass_temp = Path(name)
            ...
            for slot, pass_root in enumerate(pass_roots):
                ...
                for interval, state in enumerate(states):
                    ...
                    gain = np.abs(amplitude) ** 2
                    leakage, stream_power = repair.stream_leakage_and_power(
                        matrix,
                        q,
                    ...
                    maximum_critical_power_budget_ratio = 0.0
                    if selected_class == "SPARSE_LOCAL_STREAM_POWER_REPAIR":
                        for sector in critical:
                            ...
                            ratio = (
                                selected_power / baseline_power
                                if baseline_power > 0.0
                                else (0.0 if selected_power <= 1e-15 else math.inf)
                            )
                            maximum_critical_power_budget_ratio = max(
                                maximum_critical_power_budget_ratio, ratio
                            )
                            if selected_power > baseline_power + max(
                                1e-12, 1e-10 * max(baseline_power, 1.0)
                            ):
                                raise RuntimeError(
                                    f"pass {slot} interval {interval}: critical-sector "
                                    f"power budget exceeded for sector {sector}"
                                )
                    ...
                    action_rows.append(
                        {
                            ...
                            "maximum_critical_sector_power_budget_ratio": float(
                                maximum_critical_power_budget_ratio
                            ),
                        ...
                        affected_rows.append(
                            {
                                ...
                                "sparse_stream_status": source_choice.sparse_stream.status,
                                "sparse_stream_feasible": bool(
                                    source_choice.sparse_stream.feasible
                                ),
                    ...
                    if reviewed_choice is not None or candidate_choice is not None:
                        ...
                        if certificate_choice.sparse_stream.status == "INFEASIBLE":
                        ...
                        interval_certificates.append(
                            {
                                ...
                                "sparse_stream": solver_record(
                                    certificate_choice.sparse_stream
                                ),
                ...
                candidate_rows.append(
                    {
                        "method_id": "candidate_v4_1_floor_feasibility_repair",
                        "pass_slot": slot,
                        "protected_sample_count": int(len(long_kappa)),
                        "interval_count": int(len(states)),
        ...
        if len(affected_frame):
            for user_id, frame in affected_frame.groupby("user_id", sort=True):
                root_summary[str(user_id)] = {
                    ...
                    "sparse_stream_feasible_intervals": int(
                        frame.sparse_stream_feasible.sum()
                    ),
        ...
        if candidate_pass and reproduction_pass and formula_pass and reactive_safety_pass:
            candidate_status = (
                "PASS_EXCLUDED_SEED43999_CANDIDATE_V4_1_ALL5_FLOOR_AND_EESS_GATES"
            )
            next_gate = (
            ...
            candidate_status = (
                "FAIL_OR_INFEASIBLE_EXCLUDED_SEED43999_CANDIDATE_V4_1_REVIEW_REQUIRED"
            )
            next_gate = (
                "REVIEW_INTERVAL_CERTIFICATES_AND_CONSIDER_PROTECTED_SUBBAND_"
                "SCHEDULING_REASSIGNMENT_OR_PREREGISTERED_ADMISSION_POLICY"
            )
            exit_code = 42
        ...
        verdict = {
            ...
            "scientific_inference": {
                ...
                "preferred_repair": (
                    ...
                    "grid when feasible; otherwise power-preserving serving-sector "
                    "stream redistribution plus at most two external interferers per violated user and at most two EESS-contributor backoffs; minimum "
        ...
        status_values = {
            ...
            "ERROR_TYPE": type(exc).__name__,
            "CONFIRMATORY_CAMPAIGN_AUTHORIZED": "NO",
            "NEXT_GATE": "DIAGNOSE_FATAL_CANDIDATE_V4_1_ERROR",
        }
        write_status(output / "RUN_STATUS.env", status_values)
        make_manifest(output)
