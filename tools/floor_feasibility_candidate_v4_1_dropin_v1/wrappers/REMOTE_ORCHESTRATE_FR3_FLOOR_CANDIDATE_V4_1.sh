...
make_return_zip() {
  local code="$1"
  local mode="$2"
  local zip_name="FR3_RORQUAL_FLOOR_FEASIBILITY_CANDIDATE_V4_1_${JOB_ID}.zip"
  if [[ "$JOB_ID" == "NOT_SUBMITTED" ]]; then
    zip_name="FR3_RORQUAL_FLOOR_FEASIBILITY_CANDIDATE_V4_1_PREJOB_FAILURE.zip"
  fi
...
for path,arc in [
    ...
    (payload/'scripts/run_seed43999_floor_feasibility_candidate_v4_1.py','candidate_source/run_seed43999_floor_feasibility_candidate_v4_1.py'),
    (payload/'wrappers/REMOTE_ORCHESTRATE_FR3_FLOOR_CANDIDATE_V4_1.sh','candidate_source/REMOTE_ORCHESTRATE_FR3_FLOOR_CANDIDATE_V4_1.sh'),
    ...
    (payload/'config/CANDIDATE_V4_1_CONTRACT.json','candidate_source/CANDIDATE_V4_1_CONTRACT.json'),
    (payload/'PACKAGE_VERSION.json','candidate_source/PACKAGE_VERSION.json'),
    (run/'floor_candidate_v4_1.sbatch','slurm/floor_candidate_v4_1.sbatch'),
    ...
    (run/f'logs/floor-v4-1-{job_id}.out',f'slurm/floor-v4-1-{job_id}.out'),
    (run/f'logs/floor-v4-1-{job_id}.err',f'slurm/floor-v4-1-{job_id}.err'),
...
on_exit() {
  ...
  if (( code != 0 )); then
    ...
    if ! find "$RETURN" -maxdepth 1 -type f -name 'FR3_RORQUAL_FLOOR_FEASIBILITY_CANDIDATE_V4_1_*.zip' -print -quit | grep -q .; then
...
for path in \
  ...
  "$PAYLOAD/scripts/run_seed43999_floor_feasibility_candidate_v4_1.py" \
...
SBATCH="$RUN/floor_candidate_v4_1.sbatch"
...
#SBATCH --job-name=fr3-floor-v4-1
#SBATCH --output=$LOGS/floor-v4-1-%j.out
#SBATCH --error=$LOGS/floor-v4-1-%j.err
...
PY
if "$VENV/bin/python" "$PAYLOAD/scripts/run_seed43999_floor_feasibility_candidate_v4_1.py" \
