...
DOWNLOADS="/mnt/c/Users/alifa/Downloads"
STAMP="$(date -u +%Y%m%d_%H%M%S)"
LOCAL_RETURN="$DOWNLOADS/FR3_FLOOR_FEASIBILITY_CANDIDATE_V4_1_RETURN_$STAMP"
...
REMOTE_RUN_ROOT="$REMOTE_SCRATCH_LINK/FR3_FLOOR_FEASIBILITY_CANDIDATE_V4_1_$STAMP"
TARGET_REL="tools/floor_feasibility_candidate_v4_1_dropin_v1"
EVIDENCE_REL="evidence/phase1_floor_feasibility_candidate_v4_1_dropin_v1"
mkdir -p "$LOCAL_RETURN" "$LOCAL_CHECKS"
exec > >(tee "$LOCAL_LOG") 2>&1

CONTROL_PATH="/tmp/fr3_floor_v4_1_${UID}_$$_%C"
...
package_local_failure() {
  ...
  if find "$LOCAL_RETURN/remote_return" -maxdepth 1 -type f -name 'FR3_RORQUAL_FLOOR_FEASIBILITY_CANDIDATE_V4_1_*.zip' -print -quit 2>/dev/null | grep -q .; then return 0; fi
...
EOF_FAIL
  ...
  local zip_path="$LOCAL_RETURN/FR3_LOCAL_FLOOR_FEASIBILITY_CANDIDATE_V4_1_FAILURE_${STAMP}.zip"
...
printf '%s\n' \
  '=================================================================' \
  'FR3 FLOOR-FEASIBILITY CANDIDATE V4.1 — WSL ORCHESTRATOR' \
  '=================================================================' \
...
PY
then
  "$VENV/bin/python" -m pip install 'numpy>=1.26,<3' 'pandas>=2.1,<3' 'scipy>=1.11,<2' 'pytest>=8,<9'
fi
...
if ! git -C "$GIT_WORK" diff --cached --quiet; then
  ...
  git -C "$GIT_WORK" commit -m "Add candidate-v4.1 bounded floor-feasibility repair" >/dev/null
fi
...
PAYLOAD="$LOCAL_RETURN/FR3_FLOOR_CANDIDATE_V4_1_REMOTE_PAYLOAD.tar.gz"
...
RETURN_ZIP="$(find "$LOCAL_RETURN/remote_return" -maxdepth 1 -type f -name 'FR3_RORQUAL_FLOOR_FEASIBILITY_CANDIDATE_V4_1_*.zip' -printf '%T@ %p\n' | sort -nr | head -n1 | cut -d' ' -f2-)"
...
record={'schema_version':1,'candidate_version':'v4.1','status':'COLLECTED_FOR_INDEPENDENT_REVIEW','remote_wrapper_exit_code':remote_exit,'return_zip_sha256':zip_sha,'scientific_status':values.get('CANDIDATE_STATUS','UNKNOWN'),'confirmatory_campaign_authorized':False,'next_gate':values.get('NEXT_GATE','DIAGNOSE_REMOTE_RETURN')}
...
if ! git -C "$GIT_WORK" diff --cached --quiet; then
  git -C "$GIT_WORK" diff --cached --check
  git -C "$GIT_WORK" commit -m "Collect candidate-v4.1 excluded-seed feasibility evidence" >/dev/null
fi
...
printf '%s\n' \
  '=================================================================' \
  'FR3 FLOOR-FEASIBILITY CANDIDATE V4.1 — FINAL REPORT' \
  '=================================================================' \
