#!/usr/bin/env python3
"""Generate one campaign channel seed and evaluate all five passes/methods.
...
def require_authorization(contract: dict) -> dict:
    auth_path = os.environ.get("PHASE1_AUTHORIZATION_FILE", "")
    if not auth_path:
        raise RuntimeError(
            "PHASE1_AUTHORIZATION_FILE is absent; campaign execution is locked"
        )
    ...
    if value.get("execution_authorized") is not True:
        raise RuntimeError("authorization token does not authorize execution")
    return value
...
def pass_evaluation(
    ...
    if predictive["eligible_floor_violation_user_seconds"] != 0:
        raise RuntimeError(f"pass {slot}: predictive floor hard gate failed")
    ...
    audit = {
        ...
        "predictive_hard_gates": {
            "long_violation_seconds": predictive["long_violation_seconds"],
            "short_violation_seconds": predictive["short_violation_seconds"],
            "eligible_floor_violation_user_seconds": predictive[
...
def main() -> int:
    ...
    if args.seed is not None:
        ...
        if seed not in seeds:
            raise ValueError(f"seed not in frozen campaign list: {seed}")
        array_index = seeds.index(seed)
    else:
        array_index = (
    ...
    campaign_contract = json.loads(
        (ROOT / "PHASE1_CAMPAIGN_CONTRACT_V3.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    primary = pd.DataFrame(primary_rows).pivot(
        index="pass_slot",
        columns="method_id",
    ...
    audit = {
        "schema_version": 1,
        "status": "PASS_PHASE1_SEED_RESULT_REVIEW_REQUIRED",
        ...
        "authorization_token_sha256": sha256_file(
            Path(os.environ["PHASE1_AUTHORIZATION_FILE"])
        ),
    ...
    print("PHASE-1 SEED WORKER: PASS")
