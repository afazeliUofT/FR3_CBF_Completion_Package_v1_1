...
: "${FR3_V44_INFRA_REPAIR_ARCHIVE_PATH:?FR3_V44_INFRA_REPAIR_ARCHIVE_PATH required}"
: "${FR3_V44_INFRA_REPAIR_ARCHIVE_SHA256:?FR3_V44_INFRA_REPAIR_ARCHIVE_SHA256 required}"
...
RETURN_DIR="$FR3_DOWNLOADS/FR3_V44_SCHEDULING_INFRA_REPAIR_RETURN_$STAMP"
RUN_ROOT="$HOME/fr3_v44_scheduling_infra_repair_runs/$STAMP"
...
package_local_failure() {
  ...
  local name="FR3_V44_SCHEDULING_INFRA_REPAIR_LOCAL_FAILURE_$STAMP"
...
printf '%s\n' \
  "=================================================================" \
  "FR3 CANDIDATE V4.4 — SCHEDULING INFRASTRUCTURE REPAIR RERUN" \
  "=================================================================" \
...
"$PYTHON" "$PACKAGE_ROOT/scripts/audit_v44_infrastructure_failure.py" \
  --package-root "$PACKAGE_ROOT" \
  --output-json "$LOCAL_AUDIT_DIR/PRIOR_V44_INFRASTRUCTURE_FAILURE_AUDIT.json" \
  | tee "$LOCAL_AUDIT_DIR/PRIOR_V44_INFRASTRUCTURE_FAILURE_AUDIT.log"
...
git -C "$SOURCE_CLONE" merge-base --is-ancestor \
  639c78fd52b3d86597d6f19fc05d1478e4fd8d72 \
...
SOURCE_DEST="$SOURCE_CLONE/tools/phase1_v4_4_scheduling_infrastructure_repair_rerun_v1"
if [[ -e "$SOURCE_DEST" ]]; then
  if diff -qr "$PACKAGE_ROOT/github_payload" "$SOURCE_DEST" >/dev/null; then
    V44_REPAIR_SOURCE_COMMIT="$(git -C "$SOURCE_CLONE" rev-parse HEAD)"
    echo "V44_INFRA_REPAIR_SOURCE_ALREADY_IDENTICAL=YES"
  else
  ...
  git -C "$SOURCE_CLONE" add "tools/phase1_v4_4_scheduling_infrastructure_repair_rerun_v1"
  git -C "$SOURCE_CLONE" diff --cached --check
  git -C "$SOURCE_CLONE" -c user.name='Ali Fazeli' -c user.email='ali.fazeli@utoronto.ca' \
    commit -m 'Repair candidate-v4.4 scheduling replay infrastructure'
  git -C "$SOURCE_CLONE" fetch origin e3-first-sector-p452
...
echo "V44_INFRA_REPAIR_SOURCE_COMMIT=$V44_REPAIR_SOURCE_COMMIT"

CONTROL_PATH="/tmp/fr3-v44-r1-${UID}-$$"
...
REMOTE_UPLOAD="$RORQUAL_SCRATCH_LINK/FR3_V44_INFRA_REPAIR_${FR3_V44_INFRA_REPAIR_ARCHIVE_SHA256:0:12}.zip"
REMOTE_ORCH="$RORQUAL_SCRATCH_LINK/FR3_REMOTE_V44_INFRA_REPAIR_${FR3_V44_INFRA_REPAIR_ARCHIVE_SHA256:0:12}.sh"
scp "${SSH_OPTS[@]}" "$FR3_V44_INFRA_REPAIR_ARCHIVE_PATH" "$RORQUAL_HOST:$REMOTE_UPLOAD"
scp "${SSH_OPTS[@]}" "$PACKAGE_ROOT/wrappers/REMOTE_ORCHESTRATE_V44_SCHEDULING_INFRA_REPAIR_RERUN.sh" "$RORQUAL_HOST:$REMOTE_ORCH"

REMOTE_LOG="$RETURN_DIR/REMOTE_V44_INFRA_REPAIR.log"
RUN_TAG="${FR3_V44_INFRA_REPAIR_ARCHIVE_SHA256:0:12}"
if ssh "${SSH_OPTS[@]}" "$RORQUAL_HOST" \
  "env REMOTE_PACKAGE_ZIP='$REMOTE_UPLOAD' EXPECTED_PACKAGE_SHA256='$FR3_V44_INFRA_REPAIR_ARCHIVE_SHA256' RUN_TAG='$RUN_TAG' bash '$REMOTE_ORCH'" \
...
git -C "$EVIDENCE_CLONE" merge-base --is-ancestor "$V44_REPAIR_SOURCE_COMMIT" HEAD
ARRAY_JOB_ID="$(grep '^ARRAY_JOB_ID=' "$REMOTE_LOG" | tail -n 1 | cut -d= -f2-)"
EVIDENCE_DEST="$EVIDENCE_CLONE/evidence/phase1_v4_4_scheduling_infrastructure_repair_rerun_v1_job_${ARRAY_JOB_ID:-UNKNOWN}"
...
if [[ -e "$EVIDENCE_DEST" ]]; then
  ...
  if [[ "$existing_hash" == "$REMOTE_RETURN_SHA" ]]; then
    ...
    echo "V44_INFRA_REPAIR_EVIDENCE_ALREADY_IDENTICAL=YES"
  else
...
if [[ "$EVIDENCE_ALREADY_IDENTICAL" == "NO" ]]; then
  ...
  git -C "$EVIDENCE_CLONE" -c user.name='Ali Fazeli' -c user.email='ali.fazeli@utoronto.ca' \
    commit -m 'Collect candidate-v4.4 scheduling infrastructure-repair rerun evidence'
...
printf '%s\n' \
  "=================================================================" \
  "FR3 V4.4 SCHEDULING INFRASTRUCTURE-REPAIR FINAL REPORT" \
  "=================================================================" \
  ...
  "V44_INFRA_REPAIR_SOURCE_COMMIT=$V44_REPAIR_SOURCE_COMMIT" \
