...
: "${FR3_V45_COMPANION_ARCHIVE_PATH:?FR3_V45_COMPANION_ARCHIVE_PATH required}"
: "${FR3_V45_COMPANION_ARCHIVE_SHA256:?FR3_V45_COMPANION_ARCHIVE_SHA256 required}"
...
RETURN_DIR="$FR3_DOWNLOADS/FR3_V45_COMPANION_AWARE_RETURN_$STAMP"
RUN_ROOT="$HOME/fr3_v45_companion_aware_runs/$STAMP"
...
package_local_failure() {
  ...
  local name="FR3_V45_COMPANION_AWARE_LOCAL_FAILURE_$STAMP"
...
printf '%s\n' \
  "=================================================================" \
  "FR3 CANDIDATE V4.5 — COMPANION-AWARE SCHEDULING DEVELOPMENT" \
  "=================================================================" \
...
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH="$PACKAGE_ROOT/src" "$PYTHON" -m pytest -q -p no:cacheprovider "$PACKAGE_ROOT/tests"
...
"$PYTHON" "$PACKAGE_ROOT/scripts/audit_v44_companion_mode_gap.py" \
  --v44-return-zip "$PACKAGE_ROOT/immutable_bindings/FR3_RORQUAL_V4_4_SCHEDULING_INFRA_REPAIR_R1_18154672.zip" \
  --diagnosis-zip "$PACKAGE_ROOT/immutable_bindings/FR3_RORQUAL_V4_3_FAILED_SEED_DIAGNOSIS_18150465.zip" \
  --output-json "$LOCAL_AUDIT_DIR/CERTIFIED_V44_COMPANION_MODE_GAP_AUDIT.json" \
  | tee "$LOCAL_AUDIT_DIR/CERTIFIED_V44_COMPANION_MODE_GAP_AUDIT.log"
echo "NUMPY_JSON_SERIALIZATION_REGRESSION=PASS"
...
git -C "$SOURCE_CLONE" merge-base --is-ancestor \
  c88ecac477e085dbac533cd8117adaae83ae87a4 \
...
SOURCE_DEST="$SOURCE_CLONE/tools/phase1_v4_5_companion_aware_scheduling_development_v1"
if [[ -e "$SOURCE_DEST" ]]; then
  if diff -qr "$PACKAGE_ROOT/github_payload" "$SOURCE_DEST" >/dev/null; then
    V45_COMPANION_SOURCE_COMMIT="$(git -C "$SOURCE_CLONE" rev-parse HEAD)"
    echo "V45_COMPANION_SOURCE_ALREADY_IDENTICAL=YES"
  else
    echo "ERROR_EXISTING_V45_SOURCE_PATH_DIFFERS=$SOURCE_DEST"
    false
  fi
else
  mkdir -p "$SOURCE_DEST"
  cp -a "$PACKAGE_ROOT/github_payload/." "$SOURCE_DEST/"
  git -C "$SOURCE_CLONE" add "tools/phase1_v4_5_companion_aware_scheduling_development_v1"
  git -C "$SOURCE_CLONE" diff --cached --check
  git -C "$SOURCE_CLONE" -c user.name='Ali Fazeli' -c user.email='ali.fazeli@utoronto.ca' \
    commit -m 'Add candidate-v4.5 companion-aware scheduling development'
  git -C "$SOURCE_CLONE" fetch origin e3-first-sector-p452
...
echo "V45_COMPANION_SOURCE_COMMIT=$V45_COMPANION_SOURCE_COMMIT"

CONTROL_PATH="/tmp/fr3-v45-comp-${UID}-$$"
...
REMOTE_UPLOAD="$RORQUAL_SCRATCH_LINK/FR3_V45_COMPANION_${FR3_V45_COMPANION_ARCHIVE_SHA256:0:12}.zip"
REMOTE_ORCH="$RORQUAL_SCRATCH_LINK/FR3_REMOTE_V45_COMPANION_${FR3_V45_COMPANION_ARCHIVE_SHA256:0:12}.sh"
scp "${SSH_OPTS[@]}" "$FR3_V45_COMPANION_ARCHIVE_PATH" "$RORQUAL_HOST:$REMOTE_UPLOAD"
scp "${SSH_OPTS[@]}" "$PACKAGE_ROOT/wrappers/REMOTE_ORCHESTRATE_V45_COMPANION_AWARE_DEVELOPMENT.sh" "$RORQUAL_HOST:$REMOTE_ORCH"

REMOTE_LOG="$RETURN_DIR/REMOTE_V45_COMPANION.log"
RUN_TAG="${FR3_V45_COMPANION_ARCHIVE_SHA256:0:12}"
if ssh "${SSH_OPTS[@]}" "$RORQUAL_HOST" \
  "env REMOTE_PACKAGE_ZIP='$REMOTE_UPLOAD' EXPECTED_PACKAGE_SHA256='$FR3_V45_COMPANION_ARCHIVE_SHA256' RUN_TAG='$RUN_TAG' bash '$REMOTE_ORCH'" \
...
RETURN_VERIFY_JSON="$RETURN_DIR/V45_COMPANION_RETURN_VERIFICATION.json"
"$PYTHON" "$PACKAGE_ROOT/scripts/verify_v45_companion_aware_return.py" \
  --return-zip "$LOCAL_RETURN_ZIP" \
  --expected-sha256 "$REMOTE_RETURN_SHA" \
  --output-json "$RETURN_VERIFY_JSON" \
  | tee "$RETURN_DIR/V45_COMPANION_RETURN_VERIFICATION.log"
...
git -C "$EVIDENCE_CLONE" merge-base --is-ancestor "$V45_COMPANION_SOURCE_COMMIT" HEAD
ARRAY_JOB_ID="$(grep '^ARRAY_JOB_ID=' "$REMOTE_LOG" | tail -n 1 | cut -d= -f2-)"
EVIDENCE_DEST="$EVIDENCE_CLONE/evidence/phase1_v4_5_companion_aware_scheduling_development_v1_job_${ARRAY_JOB_ID:-UNKNOWN}"
...
if [[ -e "$EVIDENCE_DEST" ]]; then
  ...
  if [[ "$existing_hash" == "$REMOTE_RETURN_SHA" ]]; then
    ...
    echo "V45_COMPANION_EVIDENCE_ALREADY_IDENTICAL=YES"
  else
  ...
  for seed_dir in "$EVIDENCE_ROOT"/seed_results/seed_*; do
    ...
    mkdir -p "$dst"
    for name in V45_COMPANION_AWARE_SEED_RESULT.json CELL_SUMMARY.csv PRIMARY_PAIRED_EFFECTS.csv RESULT_FILE_MANIFEST.json TASK_STATUS.json process_exit_code.txt stdout.log stderr.log; do
...
if [[ "$EVIDENCE_ALREADY_IDENTICAL" == "NO" ]]; then
  ...
  git -C "$EVIDENCE_CLONE" -c user.name='Ali Fazeli' -c user.email='ali.fazeli@utoronto.ca' \
    commit -m 'Collect candidate-v4.5 companion-aware scheduling development evidence'
...
SUMMARY="$EVIDENCE_ROOT/merged/V45_COMPANION_AWARE_DEVELOPMENT_SUMMARY.json"
if [[ -f "$SUMMARY" ]]; then
...
printf '%s\n' \
  "=================================================================" \
  "FR3 V4.5 COMPANION-AWARE SCHEDULING FINAL REPORT" \
  "=================================================================" \
  ...
  "V45_COMPANION_SOURCE_COMMIT=$V45_COMPANION_SOURCE_COMMIT" \
