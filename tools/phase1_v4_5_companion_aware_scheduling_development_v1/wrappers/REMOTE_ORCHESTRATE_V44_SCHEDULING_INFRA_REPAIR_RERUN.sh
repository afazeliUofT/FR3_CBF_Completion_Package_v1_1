...
RUN_ROOT="$SCRATCH_ROOT/FR3_V44_SCHEDULING_INFRA_REPAIR_R1_${RUN_TAG}"
...
printf '%s\n' \
  ...
  "EXECUTION_STAGE=V4_4_SCHEDULING_INFRASTRUCTURE_REPAIR_RERUN" \
...
echo "REMOTE_V44_PACKAGE_SHA256_GATE=PASS"

if [[ -f "$STATE_FILE" ]]; then
  # shellcheck disable=SC1090
  source "$STATE_FILE"
  echo "EXISTING_V44_SCHEDULING_RUN_ATTACHED=YES"
else
  echo "EXISTING_V44_SCHEDULING_RUN_ATTACHED=NO"
  ...
  cp "$PACKAGE_ROOT/src/fr3_cbf/protected_subband_scheduler.py" "$JOB_ROOT/src/fr3_cbf/"
  cp "$PACKAGE_ROOT/src/fr3_cbf/candidate_v4_4_scheduling_campaign.py" "$JOB_ROOT/src/fr3_cbf/"
  {
    printf '%s  %s\n' \
      "$(sha256sum "$JOB_ROOT/src/fr3_cbf/protected_subband_scheduler.py" | awk '{print $1}')" \
      "src/fr3_cbf/protected_subband_scheduler.py"
    printf '%s  %s\n' \
      "$(sha256sum "$JOB_ROOT/src/fr3_cbf/candidate_v4_4_scheduling_campaign.py" | awk '{print $1}')" \
      "src/fr3_cbf/candidate_v4_4_scheduling_campaign.py"
  } > "$RUN_ROOT/V44_OVERLAY_MANIFEST.sha256"
  [[ "$(sha256sum "$PACKAGE_ROOT/src/fr3_cbf/protected_subband_scheduler.py" | awk '{print $1}')" == \
      "$(sha256sum "$JOB_ROOT/src/fr3_cbf/protected_subband_scheduler.py" | awk '{print $1}')" ]]
  [[ "$(sha256sum "$PACKAGE_ROOT/src/fr3_cbf/candidate_v4_4_scheduling_campaign.py" | awk '{print $1}')" == \
      "$(sha256sum "$JOB_ROOT/src/fr3_cbf/candidate_v4_4_scheduling_campaign.py" | awk '{print $1}')" ]]
  echo "REMOTE_JOB_PACKAGE_MANIFEST_VERIFICATION=PASS"
  echo "REMOTE_V44_OVERLAY_MANIFEST_VERIFICATION=PASS"
  echo "CANDIDATE_V4_3_IMMUTABLE_ARCHIVE_MODIFIED=NO"
  echo "RUN_SPECIFIC_V4_4_MODULE_OVERLAY=PASS"
  ...
  "$VENV/bin/python" "$PACKAGE_ROOT/scripts/audit_v44_infrastructure_failure.py" \
    --package-root "$PACKAGE_ROOT" \
    --output-json "$RUN_ROOT/local_audit/PRIOR_V44_INFRASTRUCTURE_FAILURE_AUDIT.json" \
  ...
  active="$(squeue -u "$USER" -h -o '%i|%j|%T' | grep -E '\|(fr3-v44-r1-sched|fr3-v44-r1-merge)\|' || true)"
  if [[ -n "$active" ]]; then
    echo "V44_SCHEDULING_CONCURRENT_RUN_GUARD=FAIL"
    printf '%s\n' "$active"
    exit 73
  fi
  echo "V44_SCHEDULING_CONCURRENT_RUN_GUARD=PASS"

  ARRAY_SBATCH="$RUN_ROOT/v44_scheduling_array.sbatch"
  cat > "$ARRAY_SBATCH" <<EOF_ARRAY
#!/usr/bin/env bash
#SBATCH --job-name=fr3-v44-r1-sched
#SBATCH --array=0-10%4
...
mkdir -p "\$OUT"
if "$VENV/bin/python" "$PACKAGE_ROOT/scripts/replay_v44_scheduling_seed.py" \
    --seed "\$SEED" \
...
EOF_ARRAY
  ...
  MERGE_SBATCH="$RUN_ROOT/v44_scheduling_merge.sbatch"
  cat > "$MERGE_SBATCH" <<EOF_MERGE
#!/usr/bin/env bash
#SBATCH --job-name=fr3-v44-r1-merge
...
if "$VENV/bin/python" "$PACKAGE_ROOT/scripts/merge_v44_scheduling_development.py" \
    --task-root "$TASK_ROOT" \
    --campaign-run-root "$CAMPAIGN_RUN_ROOT" \
...
while squeue -h -j "$MERGE_JOB_ID" | grep -q .; do
  echo "V44_SCHEDULING_PROGRESS_BEGIN"
  squeue -j "$ARRAY_JOB_ID,$MERGE_JOB_ID" -o '%.24i %.24j %.12T %.12M %.28R' || true
  echo "V44_SCHEDULING_PROGRESS_END"
  sleep 20
done
sleep 3
...
sacct -X -j "$ARRAY_JOB_ID,$MERGE_JOB_ID" \
  ...
  --parsable2 | tee "$SLURM_ROOT/sacct_v44_scheduling.txt" || true

if "$VENV/bin/python" "$PACKAGE_ROOT/scripts/package_v44_scheduling_return.py" \
    --run-root "$RUN_ROOT" \
    --package-root "$PACKAGE_ROOT" \
...
name=f"FR3_RORQUAL_V4_4_SCHEDULING_INFRA_REPAIR_R1_{array_job_id}_EMERGENCY"
...
SUMMARY="$MERGED_ROOT/V44_SCHEDULING_DEVELOPMENT_SUMMARY.json"
if [[ -f "$SUMMARY" ]]; then
