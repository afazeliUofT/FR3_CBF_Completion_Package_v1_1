"""Candidate v4.5: companion-aware protected-subband scheduling.

Candidate v4.5 preserves the predictive q-mode trajectory, fixed RZF columns,
...
from fr3_cbf import protected_subband_scheduler as v44_scheduler
from fr3_cbf import companion_aware_scheduler as companion_scheduler
...
def _schedule_scope_check(
    *,
    outcome: companion_scheduler.ScheduleOutcome,
    violating_users: np.ndarray,
    serving_bs: np.ndarray,
) -> tuple[bool, dict[str, Any]]:
    expected = tuple(
    ...
    ok = bool(
        ...
        and len(guards) <= companion_scheduler.MAX_DEPLOYABLE_GUARD_SECTORS
        and outcome.maximum_guard_sector_count
        <= companion_scheduler.MAX_DEPLOYABLE_GUARD_SECTORS
...
def run_candidate_v4_5(
    *,
    ...
    for interval, state in enumerate(states):
        ...
        schedule_outcome: companion_scheduler.ScheduleOutcome | None = None
        ...
        if hard_failure:
            ...
            if selected_class == "NO_FEASIBLE_DEPLOYABLE_ACTION_FOUND":
                ...
                common_schedule_arguments = dict(
                    gain_user_sector_stream=gain,
                ...
                schedule_outcome = v44_scheduler.schedule_interval(
                    **common_schedule_arguments
                )
                schedule_variant = "V4_4_TARGET_ONLY_LIBRARY"
                if not schedule_outcome.feasible:
                    schedule_outcome = companion_scheduler.schedule_interval(
                        **common_schedule_arguments
                    )
                    schedule_variant = "V4_5_COMPANION_AWARE_LIBRARY"
                ...
                if schedule_outcome.feasible:
                    ...
                    if schedule_variant == "V4_4_TARGET_ONLY_LIBRARY":
                        selected_class = (
                            "FLOOR_FIRST_PROTECTED_SUBBAND_SCHEDULING_"
                            f"G{schedule_outcome.guard_sector_limit}"
                        )
                    elif (
                        schedule_outcome.status
                        == "DEPLOYABLE_GUARD_SUBSET_SCHEDULING_FEASIBLE"
                    ):
                        selected_class = (
                            "FLOOR_FIRST_COMPANION_AWARE_GUARD_SUBSET_"
                            f"G{schedule_outcome.guard_sector_limit}"
                        )
                    else:
                        selected_class = (
                            "FLOOR_FIRST_COMPANION_AWARE_SCHEDULING_"
                            f"G{schedule_outcome.guard_sector_limit}"
                        )
    ...
    diagnostics: dict[str, Any] = {
        ...
        "schedule_slots_per_second": int(companion_scheduler.SLOTS_PER_SECOND),
        ...
        "maximum_protected_subband_scheduled_fraction": float(
            maximum_scheduled_slots_per_second / companion_scheduler.SLOTS_PER_SECOND
        ),
