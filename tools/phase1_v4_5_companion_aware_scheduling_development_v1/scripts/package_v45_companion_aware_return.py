#!/usr/bin/env python3
"""Package a compact, manifest-bound v4.5 scheduling development return."""
...
def main() -> int:
    ...
    name = f"FR3_RORQUAL_V4_5_COMPANION_AWARE_DEVELOPMENT_{a.array_job_id}"
    ...
    for source, relative in [
        ...
        (run_root / "local_audit/CERTIFIED_V44_COMPANION_MODE_GAP_AUDIT.json", "audits/CERTIFIED_V44_COMPANION_MODE_GAP_AUDIT.json"),
        (package_root / "src/fr3_cbf/protected_subband_scheduler.py", "candidate_source/protected_subband_scheduler.py"),
        (package_root / "src/fr3_cbf/companion_aware_scheduler.py", "candidate_source/companion_aware_scheduler.py"),
        (package_root / "src/fr3_cbf/candidate_v4_4_scheduling_campaign.py", "candidate_source/candidate_v4_4_scheduling_campaign.py"),
        (package_root / "src/fr3_cbf/candidate_v4_5_companion_aware_campaign.py", "candidate_source/candidate_v4_5_companion_aware_campaign.py"),
        (package_root / "scripts/replay_v45_companion_aware_seed.py", "candidate_source/replay_v45_companion_aware_seed.py"),
        (package_root / "scripts/merge_v45_companion_aware_development.py", "candidate_source/merge_v45_companion_aware_development.py"),
        ...
        (run_root / "slurm/sacct_v45_companion.txt", "slurm/sacct_v45_companion.txt"),
        (run_root / "V45_OVERLAY_MANIFEST.sha256", "bindings/V45_OVERLAY_MANIFEST.sha256"),
    ]:
    ...
    summary_path = run_root / "merged/V45_COMPANION_AWARE_DEVELOPMENT_SUMMARY.json"
    if summary_path.is_file():
        summary = json.loads(summary_path.read_text(encoding="utf-8"))
    else:
        summary = {
            "status": "FAIL_INFRASTRUCTURE_NO_V45_SCHEDULING_SUMMARY",
            "next_gate": "DIAGNOSE_V45_SCHEDULING_INFRASTRUCTURE",
        }
    metadata = {
        "schema_version": 1,
    ...
    print("V45_COMPANION_AWARE_RETURN_PACKAGING=PASS")
    print(f"REMOTE_RETURN_ZIP={zip_path}")
    print(f"REMOTE_RETURN_ZIP_SHA256={digest}")
    print(f"DIAGNOSTIC_SEED_RETURN_COUNT={returned}")
    print(f"V45_COMPANION_AWARE_DEVELOPMENT_STATUS={summary.get('status', 'UNKNOWN')}")
