#!/usr/bin/env python3
"""Verify complete or partial v4.5 scheduling development returns."""
...
def main() -> int:
    ...
    with zipfile.ZipFile(archive) as zf:
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            summary_path = extracted / "merged/V45_COMPANION_AWARE_DEVELOPMENT_SUMMARY.json"
            summary = (
            ...
            present = sorted(
                ...
                if (path / "result/V45_COMPANION_AWARE_SEED_RESULT.json").is_file()
    ...
    record = {
        "schema_version": 1,
        "status": (
            "PASS_COMPLETE_V45_SCHEDULING_DEVELOPMENT_RETURN"
            if complete
            else "PASS_VALID_PARTIAL_V45_SCHEDULING_DEVELOPMENT_RETURN"
        ),
        ...
        "v45_scheduling_development_status": summary.get("status"),
    ...
    print("V45_COMPANION_AWARE_RETURN_SHA256_GATE=PASS")
    print("V45_COMPANION_AWARE_RETURN_ZIP_CRC=PASS")
    print("V45_COMPANION_AWARE_RETURN_MANIFEST=PASS")
    print(f"V45_COMPANION_AWARE_RETURN_COMPLETENESS={record['return_completeness']}")
    print(f"FAILED_SEED_RETURN_COUNT={len(present)}")
    print("MISSING_FAILED_SEEDS=" + ",".join(str(v) for v in missing))
    print(f"V45_COMPANION_AWARE_DEVELOPMENT_STATUS={summary.get('status')}")
