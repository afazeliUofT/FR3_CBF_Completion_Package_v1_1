#!/usr/bin/env python3
"""Merge the 11-seed candidate-v4.5 scheduling development replay."""
...
NEW_CANDIDATE = "candidate_v4_5_companion_aware_protected_subband_scheduling"
...
def main() -> int:
    ...
    for seed in FAILED_SEEDS:
        ...
        summary_path = result_root / "V45_COMPANION_AWARE_SEED_RESULT.json"
    ...
    if missing:
        raise RuntimeError(f"missing v4.5 seed returns: {missing}")
    if len(seed_records) != len(FAILED_SEEDS):
        raise RuntimeError("v4.5 seed-result count mismatch")

    new_cells = pd.concat(new_frames, ignore_index=True)
    new_cells.to_csv(output / "V45_FAILED_SEED_CELL_SUMMARY.csv", index=False)

    # Construct the all-30 post-adaptation development/stress estimate.  The 19
    # v4.3 hard-pass seeds are a code-path identity reuse: v4.5 invokes the new
    ...
    for seed in ALL_SEEDS:
        ...
        if seed in failed_set:
            ...
            candidate_rows["development_source"] = "EXECUTED_V4_5_FAILED_SEED_REPLAY"
        else:
    ...
    development.to_csv(
        output / "V45_ALL30_DEVELOPMENT_CANDIDATE_CELLS.csv", index=False
    )
    effects.to_csv(output / "V45_ALL30_DEVELOPMENT_SEED_EFFECTS.csv", index=False)
    ...
    all30_hard_gate_pass_count = 19 + hard_pass_count
    all30_bounded_scope_pass_count = 19 + bounded_scope_count
    ...
    if original_unresolved != 1736 or schedule_success + schedule_failure != 1736:
        raise RuntimeError(
            "v4.5 scheduling coverage mismatch: "
    ...
    if all_hard:
        ...
        next_decision = (
            "FREEZE_V4_5_BEGIN_TWC_DRAFT_AND_RUN_FRESH_HOLDOUT_IN_PARALLEL"
        )
        next_gate = (
            "FREEZE_V4_5_START_13_PAGE_TWC_DRAFT_AND_RUN_FRESH_HOLDOUT_44030_44059_IN_PARALLEL"
        )
    elif diagnostic_g8 > 0 and schedule_failure == diagnostic_g8:
        status = "V4_5_DEPLOYABLE_G4_SCOPE_INSUFFICIENT_DIAGNOSTIC_G8_FEASIBLE"
        ...
        next_decision = "USER_REASSIGNMENT_OR_PREREGISTERED_ADMISSION_REQUIRED"
        next_gate = "FREEZE_DEVELOPMENT_EVIDENCE_AND_CHOOSE_REASSIGNMENT_OR_PREREGISTERED_ADMISSION_WHILE_STARTING_METHODS_DRAFT"
    else:
        status = "V4_5_COMPANION_AWARE_HARD_OR_SCOPE_GATE_REVIEW_REQUIRED"
        next_decision = "REVIEW_FAILED_HARD_OR_DISTRIBUTED_SCOPE_GATE"
        next_gate = "REVIEW_V4_5_INTERVAL_CERTIFICATES_BEFORE_FREEZE"
    ...
    summary = {
        ...
        "all30_hard_gate_pass_count": all30_hard_gate_pass_count,
        "all30_bounded_scope_pass_count": all30_bounded_scope_pass_count,
        ...
        "current_30_seed_campaign_role": (
            "DEVELOPMENT_AND_STRESS_TEST_AFTER_V4_5_ADAPTATION_NOT_CLEAN_CONFIRMATION"
        ),
        ...
        "claim_boundary": (
            "V4_5_POST_CAMPAIGN_DEVELOPMENT_RESULT_NOT_FRESH_CONFIRMATION_"
    ...
    write_json(output / "V45_COMPANION_AWARE_DEVELOPMENT_SUMMARY.json", summary)
    pd.DataFrame([{
        ...
        output / "V45_FAILED_SEED_HARD_GATE_SUMMARY.csv", index=False
    )

    print(f"V45_COMPANION_AWARE_DEVELOPMENT_STATUS={status}")
    print(f"FAILED_SEED_HARD_GATE_PASS_COUNT={hard_pass_count}")
    print(f"FAILED_SEED_BOUNDED_SCOPE_PASS_COUNT={bounded_scope_count}")
    print(f"ALL30_HARD_GATE_PASS_COUNT={all30_hard_gate_pass_count}")
    print(f"ALL30_BOUNDED_SCOPE_PASS_COUNT={all30_bounded_scope_pass_count}")
