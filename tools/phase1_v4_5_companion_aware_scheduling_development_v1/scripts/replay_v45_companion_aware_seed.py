#!/usr/bin/env python3
"""Replay candidate-v4.5 companion-aware scheduling on one preserved failed campaign seed.
...
reproduce exactly.  Candidate v4.5 is allowed to differ only because it adds the
...
def main() -> int:
    ...
    from fr3_cbf.candidate_v4_5_companion_aware_campaign import (  # type: ignore
        CANDIDATE_METHOD_ID,
        run_candidate_v4_5,
    )

    if CANDIDATE_METHOD_ID != NEW_CANDIDATE:
        raise RuntimeError("candidate-v4.5 method ID mismatch")
    worker.run_candidate_v4_3 = run_candidate_v4_5
    ...
    # Exact comparator reproduction is a source-binding gate. Candidate v4.5
    ...
    summary = {
        "schema_version": 2,
        "status": (
            "PASS_V4_5_COMPANION_AWARE_DEVELOPMENT_SEED"
            if hard_pass and bounded_scope
            else "REVIEW_V4_5_COMPANION_AWARE_DEVELOPMENT_SEED"
        ),
        ...
        "claim_boundary": (
            ...
            "V4_5_CONFIRMATION_NOT_CALIBRATION_NOT_REGULATORY_COMPLIANCE"
        ),
    }
    write_json(result_root / "V45_COMPANION_AWARE_SEED_RESULT.json", summary)
    ...
    print(
        "V4_5_COMPANION_AWARE_SEED_STATUS="
