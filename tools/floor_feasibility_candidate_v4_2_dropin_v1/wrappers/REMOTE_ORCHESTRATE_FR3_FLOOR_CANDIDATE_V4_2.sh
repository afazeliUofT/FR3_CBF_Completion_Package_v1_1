...
make_return_zip() {
  local code="$1"
  local mode="$2"
  local zip_name="FR3_RORQUAL_FLOOR_FEASIBILITY_CANDIDATE_V4_2_${JOB_ID}.zip"
  if [[ "$JOB_ID" == "NOT_SUBMITTED" ]]; then
    zip_name="FR3_RORQUAL_FLOOR_FEASIBILITY_CANDIDATE_V4_2_PREJOB_FAILURE.zip"
  fi
...
for path,arc in [
    (base/'run/results/seed_43999/channel/CHANNEL_RECORD.json','immutable_channel/CHANNEL_RECORD.json'),
    (payload/'src/fr3_cbf/floor_feasibility_repair.py','candidate_source/floor_feasibility_repair.py'),
    (payload/'scripts/run_seed43999_floor_feasibility_candidate_v4_2.py','candidate_source/run_seed43999_floor_feasibility_candidate_v4_2.py'),
    (payload/'wrappers/REMOTE_ORCHESTRATE_FR3_FLOOR_CANDIDATE_V4_2.sh','candidate_source/REMOTE_ORCHESTRATE_FR3_FLOOR_CANDIDATE_V4_2.sh'),
    (payload/'SOURCE_PAYLOAD_MANIFEST.sha256','candidate_source/SOURCE_PAYLOAD_MANIFEST.sha256'),
    (payload/'immutable_bindings/IMMUTABLE_INPUT_BINDINGS.json','candidate_source/IMMUTABLE_INPUT_BINDINGS.json'),
    (payload/'config/CANDIDATE_V4_2_CONTRACT.json','candidate_source/CANDIDATE_V4_2_CONTRACT.json'),
    (payload/'PACKAGE_VERSION.json','candidate_source/PACKAGE_VERSION.json'),
    (run/'floor_candidate_v4_2.sbatch','slurm/floor_candidate_v4_2.sbatch'),
    ...
    (run/f'logs/floor-v4-2-{job_id}.out',f'slurm/floor-v4-2-{job_id}.out'),
    (run/f'logs/floor-v4-2-{job_id}.err',f'slurm/floor-v4-2-{job_id}.err'),
...
on_exit() {
  ...
  if (( code != 0 )); then
    ...
    if ! find "$RETURN" -maxdepth 1 -type f -name 'FR3_RORQUAL_FLOOR_FEASIBILITY_CANDIDATE_V4_2_*.zip' -print -quit | grep -q .; then
...
for path in \
  ...
  "$PAYLOAD/scripts/run_seed43999_floor_feasibility_candidate_v4_2.py" \
...
SBATCH="$RUN/floor_candidate_v4_2.sbatch"
...
#SBATCH --job-name=fr3-floor-v4-2
#SBATCH --output=$LOGS/floor-v4-2-%j.out
#SBATCH --error=$LOGS/floor-v4-2-%j.err
...
PY
if "$VENV/bin/python" "$PAYLOAD/scripts/run_seed43999_floor_feasibility_candidate_v4_2.py" \
