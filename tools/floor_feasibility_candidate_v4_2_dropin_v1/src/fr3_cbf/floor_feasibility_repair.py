...
This module is a new candidate-v4.2 diagnostic/repair layer.  It does not modify
...
def stream_power_envelopes(
    ...
    The candidate repair uses the post-mode powers on the left-hand side of
    every physical power constraint and the original ``q=0`` powers only as
    the immutable sector-power envelope.
    """
    q = np.asarray(q_db, dtype=np.float64)
...
def local_sector_power_budget_system(
    *,
    ...
    The candidate's left-hand side always uses *actual post-mode* conducted
    stream powers.  The right-hand side is the reviewed fallback scalar times
    an explicit immutable envelope.  Candidate v4.2 uses the original ``q=0``
    nominal stream powers as that envelope.  Supplying post-mode powers as both
    arrays yields the stricter v4.1-style comparator and is diagnostic only.
    """
    matrix = np.asarray(mapping, dtype=np.float64)
    fixed = np.asarray(offset, dtype=np.float64)
