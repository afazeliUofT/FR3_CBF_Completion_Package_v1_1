...
def test_repair_interval_uses_q0_envelope_and_records_strict_comparator() -> None:
    ...
    assert choice.status == "PASS_EXACT_FLOOR_AND_EESS_GATES"
    assert choice.chosen_action_class == "SPARSE_LOCAL_STREAM_POWER_REPAIR"
    assert choice.strict_post_mode_sparse_stream.status == "INFEASIBLE"
    assert not choice.strict_post_mode_sparse_stream.feasible
    assert choice.sparse_stream.feasible
    actual, nominal_q0 = stream_power_envelopes(
        matrices, np.array([[10.0, 0.0]])
    )
    selected_power = float(np.sum(actual * choice.stream_scale))
    assert selected_power <= baseline * float(np.sum(nominal_q0)) + 1e-10
    assert selected_power > baseline * float(np.sum(actual)) + 1e-6
    assert choice.exact_audit.floor_violation_count == 0
