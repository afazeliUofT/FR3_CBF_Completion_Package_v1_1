...
methods, and then applies candidate-v4.2 only as a separately versioned fixed-q,
...
def load_worker(package_root: Path):
    ...
    return load_module(
        ...
        "phase1_seed_worker_candidate_v4_2_import",
    )
...
def classify_interval(choice: Any, optimistic: dict[str, Any], floor: float) -> str:
    ...
    if choice.sparse_stream.feasible:
        if not choice.strict_post_mode_sparse_stream.feasible:
            return (
                "LOCAL_SECTOR_GRID_AND_STRICT_POST_MODE_BUDGET_INFEASIBLE_"
                "Q0_NOMINAL_POWER_ENVELOPE_STREAM_REPAIR_FEASIBLE"
            )
        return (
...
def main() -> int:
    ...
    try:
        ...
        repair = load_module(candidate_source, "fr3_floor_repair_candidate_v4_2")
        ...
        with tempfile.TemporaryDirectory(prefix="fr3_candidate_v4_2_passes_") as name:
            pass_temp = Path(name)
            ...
            for slot, pass_root in enumerate(pass_roots):
                        ...
                        affected_rows.append(
                            {
                                ...
                                "q0_envelope_sparse_stream_status": (
                                    source_choice.sparse_stream.status
                                ),
                        ...
                        interval_certificates.append(
                            {
                                ...
                                "strict_post_mode_sparse_stream_comparator": solver_record(
                                    certificate_choice.strict_post_mode_sparse_stream
                                ),
                                "q0_nominal_envelope_sparse_stream": solver_record(
                ...
                candidate_rows.append(
                    {
                        "method_id": "candidate_v4_2_floor_feasibility_repair",
                        "pass_slot": slot,
                        "protected_sample_count": int(len(long_kappa)),
                        "interval_count": int(len(states)),
        ...
        candidate_pass = (
            ...
            and maximum_q0_nominal_power_envelope_ratio <= 1.0 + 1e-10
        )
        ...
        if candidate_pass and reproduction_pass and formula_pass and reactive_safety_pass:
            candidate_status = (
                "PASS_EXCLUDED_SEED43999_CANDIDATE_V4_2_ALL5_FLOOR_AND_EESS_GATES"
            )
            next_gate = (
            ...
            candidate_status = (
                "FAIL_OR_INFEASIBLE_EXCLUDED_SEED43999_CANDIDATE_V4_2_REVIEW_REQUIRED"
            )
            next_gate = (
        ...
        verdict = {
            ...
            "scientific_inference": {
                ...
                "preferred_repair": (
                    ...
                    "power redistribution bounded by the reviewed scalar times the "
                    "immutable q=0 nominal physical sector-power envelope, plus at "
            ...
            "assumptions_requiring_validation": {
                ...
                "q0_nominal_power_envelope_maps_to_available_conducted_power": True,
            ...
            "reproduction_gates": {
                ...
                "q0_power_envelope_gate": bool(
                    maximum_q0_nominal_power_envelope_ratio <= 1.0 + 1e-10
                ),
            },
        ...
        status_values = {
            ...
            "Q0_POWER_ENVELOPE_GATE": (
                "PASS"
                if maximum_q0_nominal_power_envelope_ratio <= 1.0 + 1e-10
                else "FAIL"
            ),
            "MAXIMUM_Q0_NOMINAL_POWER_ENVELOPE_RATIO": (
        ...
        status_values = {
            ...
            "ERROR_TYPE": type(exc).__name__,
            "CONFIRMATORY_CAMPAIGN_AUTHORIZED": "NO",
            "NEXT_GATE": "DIAGNOSE_FATAL_CANDIDATE_V4_2_ERROR",
        }
        write_status(output / "RUN_STATUS.env", status_values)
        make_manifest(output)
