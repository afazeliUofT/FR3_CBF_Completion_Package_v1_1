...
LOCAL_RETURN="$DOWNLOADS/FR3_FLOOR_FEASIBILITY_CANDIDATE_V4_3_RETURN_$STAMP"
...
REMOTE_RUN_ROOT="$REMOTE_SCRATCH_LINK/FR3_FLOOR_FEASIBILITY_CANDIDATE_V4_3_$STAMP"
TARGET_REL="tools/floor_feasibility_candidate_v4_3_dropin_v1"
EVIDENCE_REL="evidence/phase1_floor_feasibility_candidate_v4_3_dropin_v1"
mkdir -p "$LOCAL_RETURN" "$LOCAL_CHECKS"
exec > >(tee "$LOCAL_LOG") 2>&1

CONTROL_PATH="/tmp/fr3_floor_v4_3_${UID}_$$_%C"
...
package_local_failure() {
  ...
  if find "$LOCAL_RETURN/remote_return" -maxdepth 1 -type f -name 'FR3_RORQUAL_FLOOR_FEASIBILITY_CANDIDATE_V4_3_*.zip' -print -quit 2>/dev/null | grep -q .; then return 0; fi
...
EOF_FAIL
  ...
  local zip_path="$LOCAL_RETURN/FR3_LOCAL_FLOOR_FEASIBILITY_CANDIDATE_V4_3_FAILURE_${STAMP}.zip"
...
printf '%s\n' \
  '=================================================================' \
  'FR3 FLOOR-FEASIBILITY CANDIDATE V4.3 — WSL ORCHESTRATOR' \
  '=================================================================' \
...
echo "SOLVER_RESERVE_REGRESSION=PASS"
echo "STRICT_POST_MODE_DEPLOYMENT_POLICY_GATE=PASS"
...
PYTHONPATH="$PACKAGE_ROOT/src" "$VENV/bin/python" "$PACKAGE_ROOT/scripts/audit_prior_v4_2_result.py" --input-root "$PACKAGE_ROOT/immutable_bindings/prior_v4_2_result" --output-json "$LOCAL_CHECKS/PRIOR_V4_2_SCIENTIFIC_AUDIT.json" | tee "$LOCAL_CHECKS/PRIOR_V4_2_SCIENTIFIC_AUDIT.txt"
...
if ! git -C "$GIT_WORK" diff --cached --quiet; then
  ...
  git -C "$GIT_WORK" commit -m "Add candidate-v4.3 bounded floor-feasibility repair" >/dev/null
fi
...
PAYLOAD="$LOCAL_RETURN/FR3_FLOOR_CANDIDATE_V4_3_REMOTE_PAYLOAD.tar.gz"
...
RETURN_ZIP="$(find "$LOCAL_RETURN/remote_return" -maxdepth 1 -type f -name 'FR3_RORQUAL_FLOOR_FEASIBILITY_CANDIDATE_V4_3_*.zip' -printf '%T@ %p\n' | sort -nr | head -n1 | cut -d' ' -f2-)"
...
if [[ -f "$EXTRACTED_RETURN/candidate_source/SOURCE_PAYLOAD_MANIFEST.sha256" ]]; then
  (cd "$EXTRACTED_RETURN/candidate_source"; sha256sum -c SOURCE_PAYLOAD_MANIFEST.sha256 >/dev/null)
  echo "REMOTE_RETURN_SOURCE_MANIFEST_VERIFICATION=PASS"
  echo "RETURN_SOURCE_MANIFEST_VERIFICATION=PASS"
else
  echo "ERROR: remote return source manifest missing"
  exit 15
fi
...
record={'schema_version':1,'candidate_version':'v4.3','status':'COLLECTED_FOR_INDEPENDENT_REVIEW','remote_wrapper_exit_code':remote_exit,'return_zip_sha256':zip_sha,'scientific_status':values.get('CANDIDATE_STATUS','UNKNOWN'),'confirmatory_campaign_authorized':False,'next_gate':values.get('NEXT_GATE','DIAGNOSE_REMOTE_RETURN')}
...
if ! git -C "$GIT_WORK" diff --cached --quiet; then
  git -C "$GIT_WORK" diff --cached --check
  git -C "$GIT_WORK" commit -m "Collect candidate-v4.3 excluded-seed feasibility evidence" >/dev/null
fi
...
STRICT_POWER_GATE="$(value_from "$RUN_STATUS" STRICT_POST_MODE_SELECTED_POWER_GATE)"
...
SOLVER_RESERVE="$(value_from "$RUN_STATUS" SOLVER_CONSTRAINT_RESERVE)"
SOLVER_PRIMAL_TOL="$(value_from "$RUN_STATUS" SOLVER_PRIMAL_FEASIBILITY_TOLERANCE)"
SOLVER_DUAL_TOL="$(value_from "$RUN_STATUS" SOLVER_DUAL_FEASIBILITY_TOLERANCE)"
MAX_FLOOR_SHORTFALL="$(value_from "$RUN_STATUS" MAXIMUM_CANDIDATE_NORMALIZED_FLOOR_SHORTFALL)"
RESIDUAL_USER_COUNT="$(value_from "$RUN_STATUS" CANDIDATE_RESIDUAL_USER_COUNT)"
STRICT_UNCERTIFIED="$(value_from "$RUN_STATUS" STRICT_POST_MODE_STREAM_UNCERTIFIED_INTERVALS)"
Q0_DEPLOYED="$(value_from "$RUN_STATUS" Q0_ENVELOPE_DEPLOYABLE_ACTIONS)"
...
printf '%s\n' \
  '=================================================================' \
  'FR3 FLOOR-FEASIBILITY CANDIDATE V4.3 — FINAL REPORT' \
  '=================================================================' \
  ...
  "STRICT_POST_MODE_SELECTED_POWER_GATE=${STRICT_POWER_GATE:-UNKNOWN}" \
  ...
  "Q0_ENVELOPE_DEPLOYABLE_ACTIONS=${Q0_DEPLOYED:-UNKNOWN}" \
  "SOLVER_CONSTRAINT_RESERVE=${SOLVER_RESERVE:-UNKNOWN}" \
  "SOLVER_PRIMAL_FEASIBILITY_TOLERANCE=${SOLVER_PRIMAL_TOL:-UNKNOWN}" \
  "SOLVER_DUAL_FEASIBILITY_TOLERANCE=${SOLVER_DUAL_TOL:-UNKNOWN}" \
  "MAXIMUM_CANDIDATE_NORMALIZED_FLOOR_SHORTFALL=${MAX_FLOOR_SHORTFALL:-UNKNOWN}" \
  "CANDIDATE_RESIDUAL_USER_COUNT=${RESIDUAL_USER_COUNT:-UNKNOWN}" \
  "STRICT_POST_MODE_STREAM_UNCERTIFIED_INTERVALS=${STRICT_UNCERTIFIED:-UNKNOWN}" \
