...
def main() -> int:
    ...
    basic_checks = {
        ...
        == "PASS_RORQUAL_H100_FINAL_WORKER_ENVIRONMENT",
        "environment_exact_reference": environment.get(
            "exact_reference_environment_gate"
        ) == "PASS",
        "environment_cluster": environment.get("cluster") == "rorqual",
        "environment_hostname": "rorqual" in str(environment.get("hostname", "")).lower(),
    ...
    value = {
        ...
        "status": (
            ...
            if overall
            else "FAIL_EXCLUDED_RORQUAL_FINAL_CAMPAIGN_WORKER_SMOKE_REVIEW_REQUIRED"
        ),
        "claim_boundary": (
        ...
        "next_gate": (
            "BUILD_AND_INDEPENDENTLY_REVIEW_RORQUAL_NATIVE_LOCKED_30_SEED_"
            "CAMPAIGN_PACKAGE"
            if overall
            else "DIAGNOSE_FINAL_CAMPAIGN_WORKER_OR_RORQUAL_CHANNEL_REPRODUCTION_"
