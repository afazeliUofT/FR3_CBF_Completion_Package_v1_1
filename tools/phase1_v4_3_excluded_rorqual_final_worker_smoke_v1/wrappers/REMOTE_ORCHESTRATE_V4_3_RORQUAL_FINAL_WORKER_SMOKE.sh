...
: "${SCRATCH_ROOT_ABS:?SCRATCH_ROOT_ABS is required}"
...
[[ "$HOST_FULL" == *rorqual* ]] || {
  echo "ERROR_REMOTE_HOST_NOT_RORQUAL=$HOST_FULL"
  exit 20
}
SCRATCH_ROOT="$(realpath -e "$SCRATCH_ROOT_ABS" 2>/dev/null || true)"
[[ -n "$SCRATCH_ROOT" && -d "$SCRATCH_ROOT" ]] || {
  echo "ERROR_RORQUAL_SCRATCH_RESOLUTION_FAILED=$SCRATCH_ROOT_ABS"
  exit 21
}
...
build_emergency_return() {
  local code="$1"
  local output="$RETURN_HOME/FR3_RORQUAL_V4_3_FINAL_WORKER_SMOKE_43999_${JOB_ID}_EMERGENCY.zip"
...
# Record the already-completed excluded Rorqual H100 deployment smoke.
{
  echo "PRIOR_RORQUAL_H100_SMOKE_18115499_STATUS_CHECK_BEGIN"
  sacct -X -j 18115499 \
    --format=JobID,JobName,State,ExitCode,Elapsed,MaxRSS,ReqMem,AllocTRES -P \
    2>&1 || true
  echo "PRIOR_RORQUAL_H100_SMOKE_18115499_STATUS_CHECK_END"
} | tee "$PROVENANCE/prior_rorqual_h100_smoke_18115499_status.txt"
...
if [[ -x "$VENV/bin/python" ]] && exact_environment_gate; then
  ENV_SETUP_RC=0
  echo "RORQUAL_EXACT_REFERENCE_ENVIRONMENT_REUSE=PASS"
else
  echo "RORQUAL_EXACT_REFERENCE_ENVIRONMENT_REPAIR_REQUIRED=YES"
  ...
  echo "RORQUAL_ENVIRONMENT_SETUP_EXIT_CODE=$ENV_SETUP_RC"
  [[ "$ENV_SETUP_RC" -eq 0 ]]
  exact_environment_gate
  echo "RORQUAL_EXACT_REFERENCE_ENVIRONMENT_REPAIR=PASS"
fi
...
printf '%s\n' "${ASSOCIATIONS[@]:-}" > "$PROVENANCE/rorqual_account_associations.txt"
ACCOUNT=""
...
[[ -n "$ACCOUNT" ]] || {
  echo "ERROR_NO_RORQUAL_ACCOUNT_SELECTED=YES"
  exit 25
}
...
JOB_ID="$(
  sbatch \
    ...
    "$SMOKE_ROOT/wrappers/RORQUAL_FINAL_WORKER_SMOKE_H100.sh"
)"
...

{
  ...
  echo "CHANNEL_GENERATED_ON_RORQUAL=YES"
...
RETURN_NAME="FR3_RORQUAL_V4_3_FINAL_WORKER_SMOKE_43999_${JOB_ID}.zip"
