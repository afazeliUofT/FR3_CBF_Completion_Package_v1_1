...
echo "RORQUAL_JOB_PACKAGE_MANIFEST_VERIFICATION=PASS"
echo "RORQUAL_SMOKE_PACKAGE_MANIFEST_VERIFICATION=PASS"
...
python "$SMOKE_ROOT/scripts/capture_rorqual_h100_environment.py" \
  --output "$PROVENANCE/RORQUAL_H100_ENVIRONMENT.json"
...
if [[ -d "$SEED_ROOT/result" && -f "$SEED_ROOT/channel/CHANNEL_RECORD.json" ]]; then
  if "$VENV/bin/python" "$SMOKE_ROOT/scripts/audit_final_worker_smoke_result.py" \
      ...
      --environment "$PROVENANCE/RORQUAL_H100_ENVIRONMENT.json" \
...
if [[ "$WORKER_RC" -eq 0 \
   ...
   && "$AUDIT_RC" -eq 0 ]]
then
  echo "RORQUAL_FINAL_WORKER_SMOKE_JOB=PASS"
  exit 0
fi

if [[ "$WORKER_RC" -eq 42 || "$AUDIT_RC" -eq 42 ]]; then
  echo "RORQUAL_FINAL_WORKER_SMOKE_JOB=SCIENTIFIC_FAIL_RETURN_READY"
  exit 42
fi

echo "RORQUAL_FINAL_WORKER_SMOKE_JOB=INFRASTRUCTURE_OR_VALIDATION_FAIL_RETURN_READY"
