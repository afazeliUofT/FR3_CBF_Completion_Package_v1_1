        ...
        json.dumps(
            {
                ...
                "cluster": "rorqual",
                "hostname": "rg-test.rorqual.alliancecan.ca",
                "actual_versions": {
                    "torch": "2.9.1",
                    "sionna-no-rt": "2.0.1",
                    "numpy": "2.4.2",
...
def test_locked_campaign_inputs_audit(tmp_path: Path) -> None:
    ...
    canonical = json.loads(
        (ROOT / "immutable_bindings/LOCKED_CAMPAIGN_CANONICAL_PATHS.json").read_text(
            encoding="utf-8"
        )
    )
    routing = json.loads(
        (ROOT / "immutable_bindings/ROUTING_CORRECTION_RECORD.json").read_text(
            encoding="utf-8"
        )
    )
    assert contract["execution"]["cluster"] == "rorqual"
    assert contract["required_git_ancestor"] == "76603de75d9a1c45555d94443b5494a41275b7bb"
    assert canonical["job_package_path"].startswith(
        "campaign/phase1_nibi_job_package_v4_3_v1/"
    )
    assert routing["superseded_nibi_job_id"] == "18953376"
    assert routing["correct_cluster"] == "rorqual"
...
def test_exact_synthetic_final_worker_audit_and_tamper_detection(tmp_path: Path) -> None:
    ...
    value = json.loads(output.read_text(encoding="utf-8"))
    assert value["status"] == "PASS_EXCLUDED_RORQUAL_FINAL_CAMPAIGN_WORKER_SMOKE_SEED43999"
...
def test_success_return_excludes_token_and_raw_channel(tmp_path: Path) -> None:
    ...
    (provenance / "RORQUAL_H100_ENVIRONMENT.json").write_text(
...
def test_local_failure_return_is_portable(tmp_path: Path) -> None:
    ...
    assert completed.returncode == 0
    archive = local / "FR3_V4_3_RORQUAL_FINAL_WORKER_SMOKE_LOCAL_FAILURE.zip"
...
def test_diagnostic_return_is_retrievable_and_non_authorizing(tmp_path: Path) -> None:
    ...
    assert "RETURN_STATUS=DIAGNOSTIC_EXCLUDED_RORQUAL_FINAL_WORKER_SMOKE_RETURN_READY" in validated.stdout
    with zipfile.ZipFile(output) as bundle:
        names = bundle.namelist()
...
def test_wrappers_enforce_one_excluded_job_and_no_campaign_execution() -> None:
    rorqual = (ROOT / "wrappers/RORQUAL_FINAL_WORKER_SMOKE_H100.sh").read_text()
    remote = (
    ...
    combined = rorqual + remote + wsl
    assert "--seed \"$SEED\"" in rorqual
    assert "SEED=43999" in rorqual
    assert "--gpus=h100:1" in remote
    ...
    assert "--reuse-channel" not in rorqual
    assert "rm -f \"$TOKEN\"" in remote
    assert "NO_PRIOR_FINAL_WORKER_SMOKE_EVIDENCE_GATE=PASS" in wsl
    assert 'SUPERSEDED_NIBI_JOB_ID="18953376"' in wsl
    assert 'scancel "$job"' in wsl
    assert "SUPERSEDED_NIBI_AUTHORIZATION_TOKEN_DELETED=PASS" in wsl
    assert "SUPERSEDED_NIBI_RESULT_CLASSIFICATION=ROUTING_CORRECTION_NOT_SCIENTIFIC_EVIDENCE" in wsl
    assert wsl.index("SUPERSEDED_NIBI_SMOKE_CANCELLATION_BEGIN=YES") < wsl.index("RORQUAL_CONNECTION=PASS")
    assert "CHANNEL_GENERATED_ON_NIBI=YES" not in combined
    assert "REMOTE_ORCHESTRATE_V4_3_NIBI" not in combined
    assert "/home/rsadve1/links/scratch" in wsl
