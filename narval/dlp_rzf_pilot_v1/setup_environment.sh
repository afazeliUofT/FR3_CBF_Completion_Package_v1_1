...
WHEELS="$BASE/wheels"
mkdir -p "$BASE" "$WHEELS" "$(dirname "$VENV")"
...
if [[ -d "$VENV" ]] && ! environment_matches; then
  STAMP="$(date -u +%Y%m%d_%H%M%S)"
  mv "$VENV" "${VENV}_incompatible_${STAMP}"
  echo "Archived incompatible Narval virtual environment."
fi
...
print("NARVAL PILOT ENVIRONMENT SETUP: PASS")
...
print("PanelArray ports:", array.num_ant)
PY
...
echo "Narval virtual environment: $VENV"
