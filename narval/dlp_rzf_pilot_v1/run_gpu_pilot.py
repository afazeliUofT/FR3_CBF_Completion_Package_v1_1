...
def steering_modes(array, local_direction_xyz, frequency_hz: float, device: str):
    """Build two orthogonal Sionna-port steering modes from a frozen local vector."""
    ...
    # Sionna's transmit channel row uses exp(+j k r dot d).  The
    # leakage constraint is written ||a^H W||^2, hence a is the Hermitian
    # steering column exp(-j k r dot d).
...
def main() -> int:
    ...
    if props.total_memory < 38 * 1024**3:
        raise RuntimeError(
            f"Expected a full 40 GB-class A100, found "
            f"{props.total_memory/1024**3:.1f} GiB"
        )
    if "A100" not in torch.cuda.get_device_name(0):
        raise RuntimeError(
            f"Narval pilot requires an A100; found "
    ...
    if port_position_error > 2e-7:
        raise ValueError(
            f"Narval Sionna port positions differ from the audited CPU order: "
    ...
    if not np.array_equal(actual_pol1, expected_pol1):
        raise ValueError("Narval polarization-1 port order differs from the audit")
    if not np.array_equal(actual_pol2, expected_pol2):
        raise ValueError("Narval polarization-2 port order differs from the audit")

    frame_audit = json.loads(
        (INPUT / "SIONNA_INCUMBENT_LOCAL_FRAME_AUDIT.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    frame_reference = pd.read_csv(INPUT / "SIONNA_INCUMBENT_LOCAL_FRAME.csv")
    if len(frame_reference) != cfg["sector_count"] or frame_reference["sector_id"].nunique() != cfg["sector_count"]:
        raise ValueError("Incumbent local-frame reference must contain 57 unique sectors")
    frame_reference = frame_reference.set_index("sector_id")
    direction_norms = np.linalg.norm(
        frame_reference[["local_direction_x", "local_direction_y", "local_direction_z"]].to_numpy(float),
        axis=1,
    )
    if not np.allclose(direction_norms, 1.0, rtol=0.0, atol=2e-12):
        raise ValueError("A frozen incumbent local direction is not unit norm")
    ...
    frequency_count = len(cfg["frequency_offsets_hz"])
    h = torch.empty(
        (
            cfg["user_count"],
            cfg["sector_count"],
    ...
    coefficient_hasher = hashlib.sha256()
    delay_hasher = hashlib.sha256()
    response_hasher = hashlib.sha256()
    chunk_size = int(cfg.get("channel_user_chunk_size", 8))
    if chunk_size <= 0:
        raise ValueError("channel_user_chunk_size must be positive")
    chunk_records = []
    for start_user in range(0, cfg["user_count"], chunk_size):
        stop_user = min(start_user + chunk_size, cfg["user_count"])
        model.set_topology(
            ut_loc[:, start_user:stop_user],
            bs_loc,
            ut_ori[:, start_user:stop_user],
            bs_ori,
            ut_vel[:, start_user:stop_user],
            in_state[:, start_user:stop_user],
        )
        coefficients_chunk, delays_chunk = model(
            num_time_samples=1,
            sampling_frequency=15_000.0,
        )
        offsets = torch.tensor(
            cfg["frequency_offsets_hz"],
            dtype=delays_chunk.dtype,
            device=device,
        )
        h_chunk_full = cir_to_ofdm_channel(
            offsets,
            coefficients_chunk,
            delays_chunk,
            normalize=False,
        )
        h_chunk = h_chunk_full[0, :, 0, :, :, 0, :]
        ...
        if tuple(h_chunk.shape) != expected_chunk_shape:
            raise ValueError(
                f"Unexpected channel chunk shape {tuple(h_chunk.shape)}; "
                f"expected {expected_chunk_shape}"
            )
        h[start_user:stop_user] = h_chunk
        coefficients_cpu = (
            coefficients_chunk.detach().cpu().contiguous().numpy()
        )
        delays_cpu = delays_chunk.detach().cpu().contiguous().numpy()
        response_cpu = h_chunk.detach().cpu().contiguous().numpy()
        coefficient_hasher.update(coefficients_cpu.tobytes())
        delay_hasher.update(delays_cpu.tobytes())
        response_hasher.update(response_cpu.tobytes())
        chunk_records.append(
            {
                "start_user": start_user,
                "stop_user": stop_user,
                "coefficient_shape": list(coefficients_chunk.shape),
                "delay_shape": list(delays_chunk.shape),
                "response_shape": list(h_chunk.shape),
            }
        )
        del coefficients_chunk, delays_chunk, h_chunk_full, h_chunk
        del coefficients_cpu, delays_cpu, response_cpu
    ...
    if tuple(h.shape) != (
        ...
        raise ValueError(f"Unexpected assembled channel shape: {tuple(h.shape)}")
    ...
    for b, row in bs.iterrows():
        ...
        frame_row = frame_reference.loc[sector_id]
        a1, a2 = steering_modes(
            array,
            [
                float(frame_row["local_direction_x"]),
                float(frame_row["local_direction_y"]),
                float(frame_row["local_direction_z"]),
            ],
    ...
    write_json(OUTPUT / "NARVAL_GPU_ENVIRONMENT.json", environment)
    audit = {
        "created_utc": datetime.now(timezone.utc).isoformat(),
        ...
        "dimensions": {
            ...
            "channel_user_chunk_size": chunk_size,
            "channel_chunk_count": len(chunk_records),
        },
        "safety": {
        ...
        "standards_and_port_gates": {
            ...
            "incumbent_local_frame_status": frame_audit["status"],
            "incumbent_steering_coordinate_model": cfg["incumbent_steering_coordinate_model"],
            "transmit_polarization_bound": cfg["transmit_polarization_bound"],
            "steering_vector_convention": (
                "a=exp(-j k r_local dot d_local); a^H matches the Sionna "
                "transmit channel-row array phase"
            ),
            "superseded_bundle_sha256": frame_audit["superseded_bundle"]["sha256"],
        },
        "channel_generation": {
            "mode": "sequential_user_chunks_to_fit_one_A100_40GB",
            "chunk_size": chunk_size,
            "chunk_count": len(chunk_records),
            "spatial_consistency_across_user_chunks": False,
        },
        "limitations": [
            "Channel generation is chunked by user to fit one Narval A100; cross-user large-scale spatial correlation across chunks is not claimed.",
            "The transmit polarization protection calculation is a conservative sum of the two orthogonal Sionna port-group spatial-mode powers, not an exact H-polarized far-field reconstruction.",
        ...
        "fingerprints": {
            "coefficients_chunk_stream_sha256": coefficient_hasher.hexdigest(),
            "delays_chunk_stream_sha256": delay_hasher.hexdigest(),
    ...
    write_json(OUTPUT / "NARVAL_ONE_SEED_DLP_RZF_PILOT_AUDIT.json", audit)
    print("NARVAL ONE-SEED 57-SECTOR 4-USER DLP-RZF PILOT: PASS")
